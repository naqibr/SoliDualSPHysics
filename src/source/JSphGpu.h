...
protected:
	...
	StDivDataGpu DSDivData;		///<Current data of cell division for neighborhood search on GPU for deformbale structures.
	...
	JUserExpressionListGPU* UserExpressionsg = nullptr;
	//=======================================================================================================
	///Deformable structure variables
	//=======================================================================================================
	StDeformStrucIntData* DefStrucIntDatac;	///<Data for each individual deformable structure body on CPU [DeformStrucCount]
	StDeformStrucIntData* DefStrucIntDatag;	///<Data for each individual deformable structure body on GPU [DeformStrucCount]
	StDeformStrucIntArraysg DSIntArraysg;
	unsigned* DeformStrucRidpg;				///<Identifier to access to the particles of the deformable structures [CaseNdeformstruc].
	unsigned* DeformStrucRidpc;				///<Identifier to access to the particles of the deformable structures [CaseNdeformstruc].
	unsigned* DSBestChildg;					///<List of particles in the mapped bodies that are close to or at the center of ith particle of the original domain [CaseNdeformstruc].
	float2* DSPosOrg0xyg;					///<List of initial positions of original deformable structures domain [CaseNdeformstruc]
	tfloat2* DSPosOrg0xyc;					///<List of initial positions of original deformable structures domain [CaseNdeformstruc]
	float* DSPosOrg0zg;						///<List of initial positions of original deformable structures domain [CaseNdeformstruc]
	float* DSPosOrg0zc;						///<List of initial positions of original deformable structures domain [CaseNdeformstruc]
	float* DSKerg;							///<List of kernel values between solid structure particles [DSNumPairs].
	float4* DSKerDerLapg;					///<List of kernel x,y,z: derivative and w:laplacian values between solid structure particles [DSNumPairs].
	unsigned* DSPairJg;						///<List of j (neighbour) index to each initial neighbourlist of all the mapped domain [DSNumPairs].
	unsigned* DSDcellg;						///<Cells inside DSDomCells coded with DSDomCellCode for gpu.
	unsigned* DSDcellc;						///<Cells inside DSDomCells coded with DSDomCellCode for cpu.

	unsigned* DSSurfPartListg;				///<List of particles on the surface of mapped deformable bodies[DSNpSurf]
	uint2* DSPairNSg;						///<x:Number of pairs and y: address of neighbours of a particle in the kernelderlap array [MapNdeformstruc].
	tbcstruc* DSPartVBCg;					///<Particle array with informations of velocity boundary condition [MapNdeformstruc].
	tbcstruc* DSPartFBCg;					///<Particle array with informations of force boundary condition [MapNdeformstruc].
	tphibc* DSPartPhiBCg;					///<Particle array with compact phi BC (8 bytes: exprid + flags) [MapNdeformstruc].
	float* DSKerSumVolg;					///<Volume of particle based on kernel summation [MapNdeformstruc].
	float* DSKerSumVolc;					///<Volume of particle based on kernel summation [MapNdeformstruc].
	float4* DSPos0g;						///<Initial particle positions and w: current strain energy in mapped domain [MapNdeformstruc].
	tfloat4* DSPos0c;						///<Initial particle positions in mapped domain [MapNdeformstruc].
	typecode* DSCodeg;						///<Indicator of group of particles & other special markers. [MapNdeformstruc].
	typecode* DSCodec;						///<Indicator of group of particles & other special markers. [MapNdeformstruc].
	float4* DSDispPhig;						///<Particle x,y,z: displacement and w:phase field in mapped domain [MapNdeformstruc].
	float* DSEqPlasticg;					///<Equivalent plastic strain per particle in mapped domain [MapNdeformstruc].
	tmatrix3f* DSPlasticStraing;			///<Deviatoric plastic strain tensor per particle in mapped domain [MapNdeformstruc].
	matrix3f* DSPlasticFpg;
	tfloat4* DSDispPhic;					///<Particle displacement in mapped domain [MapNdeformstruc]. used for visualization
	float* DSEqPlasticc;					///<Equivalent plastic strain per particle on CPU [MapNdeformstruc].
	tmatrix3f* DSPlasticStraingc;			///<Deviatoric plastic strain tensor per particle on CPU [MapNdeformstruc].
	float4* DSVelg;							///<Particle velocity and kinetic energy in mapped domain [MapNdeformstruc].
	float4* DSVelPreg;						///<Particle velocity and phidot pre for symplectic in mapped domain [MapNdeformstruc].
	float4* DSAcclg;						///<Particle acceleration and w: optimal time step in mapped domain [MapNdeformstruc].
	float3* DSFlForceg;						///<Particle acceleration coming from fluid
	tfloat3* DSFlForcec;					///<Particle acceleration coming from fluid
	...
	unsigned* DSParentg;					///<Id (as in Posc[id]) of particle to which the mapped particle is part [MapNdeformstruc].
	unsigned* DSParentc;					///<Id (as in Posc[id]) of particle to which the mapped particle is part on cpu [MapNdeformstruc].
	unsigned* DSiBodyRidpg;					///<Identifier to access each deformable structure particles on gpu [MapNdeformstruc].
	unsigned* DSiBodyRidpc;					///<Identifier to access each deformable structure particles on cpu [MapNdeformstruc].
	float* DSblockMinMax;					///Array to store any min or max value of deformable structure domain
	unsigned* DSMeasPlnCntc;					///<Per plane count of particles located on the measuring plane.
	unsigned* DSMeasPlnPartc;				///<Particle list on all measuring planes
	...
	void DSConfigCellDiv(JCellDivGpu* celldiv) { DSCellDiv = celldiv; };
	void InitFloating();
	void InitRunGpu();
