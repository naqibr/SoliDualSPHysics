...
StGpuInfo GetCudaDeviceInfo(int gid){
  ...
  const auto getattr=[gid](cudaDeviceAttr attr,int defaultvalue){
    int value=defaultvalue;
    if(cudaDeviceGetAttribute(&value,attr,gid)!=cudaSuccess)value=defaultvalue;
    return(value);
  };
  g.clockrate=getattr(cudaDevAttrClockRate,0);
#if CUDART_VERSION >= 5000
  g.clockratemem=getattr(cudaDevAttrMemoryClockRate,0);
  ...
  g.overlap=getattr(cudaDevAttrGpuOverlap,0);
  g.overlapcount=deviceProp.asyncEngineCount;
  g.limitrun=getattr(cudaDevAttrKernelExecTimeout,0);
