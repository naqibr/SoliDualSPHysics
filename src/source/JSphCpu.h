...
protected:
	...
	StDivDataCpu DSDivData;		///<Current data of cell division for neighborhood search on CPU for deformbale structures.
	...
	void DSConfigCellDiv(JCellDivCpu* celldiv) { DSCellDiv = celldiv; };
	void InitFloating();
	void InitRunCpu();
	...
	void DSAllocCpuMemoryFixed();
	unsigned DSCountTotalPairs(StDeformStrucData* defstrucdatac);
	bool DSLineSurfIntersect(const tdouble3 part1, const tdouble3 part2, const plane4Nstruc* rect4n, const unsigned size4n)const;
	template<TpKernel tker> void DSCalcKers_ct(StDeformStrucData* defstrucdatac)const;
	void DSCalcKers(StDeformStrucData* defstrucdatac)const;
	float DSGetSelfKernBody(float bkernh)const;
	void DSApplyFluidPressure(const double dt);
	//=======================================================================================================
	///Deformable structure variables
	//=======================================================================================================
	StDeformStrucIntData* DefStrucIntData;	///<Data for each individual deformable structure body [DeformStrucCount]
	unsigned* DeformStrucRidp;				///<Identifier to access to the particles of the deformable structures [CaseNdeformstruc].
	unsigned* DSBestChild;					///<List of particles in the mapped bodies that are close to or at the center of ith particle of the original domain [CaseNdeformstruc].
	tdouble3* DSPosOrg0;					///<List of initial positions of original deformable structures domain [CaseNdeformstruc]
	double* DSKer;							///<List of kernel values between solid structure particles [DSNumPairs].
	tdouble3* DSKerDerV0;					///<List of kernel derivative*vol0 values between solid structure particles [DSNumPairs].
	double* DSKerLapc;						///<List of laplacian operators between solid structure particles [DSNumPairs]. laplacian = Sum (Fi-Fj) * KerLapcij
	unsigned* DSPairJ;						///<List of j index to each initial neighbourlist of all the mapped domain [DSNumPairs].
	unsigned* DSDcellc;						///<Cells inside DSDomCells coded with DSDomCellCode.
	unsigned* DSDcellSurfc;					///<Cells inside DSDomCells coded with DSDomCellCode.

	unsigned* DSSurfPartList;				///<List of particles on the surface of mapped deformable bodies[DSNpSurf]
	unsigned* DSPairStart;					///<Address of the start of pair lists in all the mapped bodies [MapNdeformstruc].
	unsigned* DSPairN;						///<Number of pairs of a particle in all the mapped bodies [MapNdeformstruc].
	tbcstruc* DSPartVBC;					///<Particle array with informations of velocity boundary condition [MapNdeformstruc].
	tbcstruc* DSPartFBC;					///<Particle array with informations of force boundary condition [MapNdeformstruc].
	tphibc* DSPartPhiBC;					///<Particle array with compact phi BC (8 bytes: exprid + flags) [MapNdeformstruc].
	float* DSKerSumVol;						///<Volume of particle based on kernel summation [MapNdeformstruc].
	tdouble3* DSPos0;						///<Initial particle positions in mapped domain [MapNdeformstruc].
	typecode* DSCode;						///<///<Indicator of group of particles & other special markers. [MapNdeformstruc].
	tdouble3* DSDisp;						///<Particle displacement in mapped domain [MapNdeformstruc].
	tdouble3* DSVel;						///<Particle velocities in mapped domain [MapNdeformstruc].
	tdouble3* DSVelPre;						///<Particle velocity Pre for symplectic in mapped domain [MapNdeformstruc].
	tdouble3* DSAccl;						///<Particle acceleration in mapped domain [MapNdeformstruc].
	tdouble3* DSFlForce;					///<Particle acceleration due to fluid pressure in mapped domain [MapNdeformstruc].
	tmatrix3d* DSDefGrad;					///<Particle deformation gradient in mapped domain [MapNdeformstruc].
	tmatrix3d* DSPiolKir;					///<Particle Piolla-Kirchhof Stress in mapped domain [MapNdeformstruc].
	//tdouble3* DSEnergies;					///<Energies of the particles, x: strain, y:kinetic, z: fracture in mapped domain [MapNdeformstruc].
	double* DSPhi;							///<Particle phase field values in mapped domain [MapNdeformstruc].
	double* DSPhiDotPre;					///<Particle phase field dot Pre values for symplectic in mapped domain [MapNdeformstruc].
	double* DSPhiDot;						///<Particle phase field time derivative values in mapped domain [MapNdeformstruc].
	double* DSPhiDDot;						///<Particle phase field time derivative2 in mapped domain [MapNdeformstruc].
	double* DSHis;							///<Particle history variable for phase field in mapped domain [MapNdeformstruc].
	tdouble3* DSCorrArray;					///<An Array to store correction variables in mapped domain [MapNdeformstruc].
	double* DSJacob;						///<Particle determinant of deformation gradients
	double* DSEqPlastic;					///<Equivalent plastic strain per particle [MapNdeformstruc].
	tmatrix3d* DSPlasticStrain;				///<Deviatoric plastic strain tensor per particle [MapNdeformstruc].
	tmatrix3d* DSPlasticFp;				///<Deviatoric plastic strain tensor per particle [MapNdeformstruc].
	unsigned* DSParent;						///<Id (as in Posc[id]) of particle to which the mapped particle is part [MapNdeformstruc].
	unsigned* DSiBodyRidp;					///<Identifier to access each deformable structure particles [MapNdeformstruc].
	tmatrix3d DSIdentc;						///<Identity matrix
	unsigned* DSMeasPlnCnt;					///<Per plane count of particles located on the measuring plane.
	unsigned* DSMeasPlnPart;				///<Particle list on all measuring planes
	tdouble3 arrminmax[OMP_MAXTHREADS * OMP_STRIDE];	///<tdouble3 array to store temporary min or max values accross threads
public:
