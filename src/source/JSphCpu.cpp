...
void JSphCpu::InitVars() {
	...
	//=======================================================================================================
	///Deformable structure variables
	//=======================================================================================================
	DefStrucIntData = NULL; DeformStrucRidp = NULL; DSBestChild = NULL; DSPosOrg0 = NULL; DSKer = NULL;
	DSKerDerV0 = NULL; DSKerLapc = NULL; DSPairJ = NULL; DSDcellc = NULL; DSDcellSurfc = NULL; DSSurfPartList = NULL;
	DSPairStart = NULL; DSPairN = NULL; DSPartVBC = NULL; DSPartFBC = NULL; DSKerSumVol = NULL;
	DSPos0 = NULL; DSCode = NULL; DSDisp = NULL; DSVel = NULL; DSVelPre = NULL; DSAccl = NULL;
	DSDefGrad = NULL; DSPiolKir = NULL; DSPhi = NULL; DSPhiDot = NULL; DSPhiDotPre = NULL;
	DSPhiDDot = NULL; DSHis = NULL; DSCorrArray = NULL; DSJacob = NULL; DSEqPlastic = NULL; DSPlasticStrain = NULL; DSParent = NULL;
	DSiBodyRidp = NULL;
...
void JSphCpu::FreeCpuMemoryFixed() {
	...
	//Deformable structure arrays
	delete[]DefStrucIntData; DefStrucIntData = NULL;
	delete[]DeformStrucRidp; DeformStrucRidp = NULL;
	delete[]DSBestChild; DSBestChild = NULL;
	delete[]DSPosOrg0; DSPosOrg0 = NULL;
	delete[]DSKer; DSKer = NULL;
	delete[]DSKerDerV0; DSKerDerV0 = NULL;
	delete[]DSKerLapc; DSKerLapc = NULL;
	delete[]DSPairJ; DSPairJ = NULL;
	delete[]DSDcellc; DSDcellc = NULL; 
	delete[]DSDcellSurfc; DSDcellSurfc = NULL;
	delete[]DSSurfPartList; DSSurfPartList = NULL;
	delete[]DSPairStart; DSPairStart = NULL;
	delete[]DSPairN; DSPairN = NULL;
	delete[]DSPartVBC; DSPartVBC = NULL;
	delete[]DSPartFBC; DSPartFBC = NULL;
	delete[]DSKerSumVol; DSKerSumVol = NULL;
	delete[]DSPos0; DSPos0 = NULL;
	delete[]DSCode; DSCode = NULL;
	delete[]DSDisp; DSDisp = NULL;
	delete[]DSVel; DSVel = NULL;
	if (DSVelPre) {
		delete[]DSVelPre; DSVelPre = NULL;
	}
	delete[]DSAccl; DSAccl = NULL;
	delete[]DSDefGrad; DSDefGrad = NULL;
	delete[]DSPiolKir; DSPiolKir = NULL;
	delete[]DSPhi; DSPhi = NULL;
	delete[]DSPhiDot; DSPhiDot = NULL;
	if (DSPhiDotPre) {
		delete[]DSPhiDotPre; DSPhiDotPre = NULL;
	}
	delete[]DSPhiDDot; DSPhiDDot = NULL;
	delete[]DSHis; DSHis = NULL;
	delete[]DSCorrArray; DSCorrArray = NULL;
	delete[]DSJacob; DSJacob = NULL;
	delete[]DSEqPlastic; DSEqPlastic = NULL;
	delete[]DSPlasticStrain; DSPlasticStrain = NULL;
	delete[]DSParent; DSParent = NULL;
	delete[]DSiBodyRidp; DSiBodyRidp = NULL;
	if (DSPartPhiBC) {
		free(DSPartPhiBC); DSPartPhiBC = NULL;
	}
}
...
//==============================================================================
/// Allocates memory for arrays with fixed size Deformable structure.
//==============================================================================
void JSphCpu::DSAllocCpuMemoryFixed() {

	if (MapNdeformstruc) {
		DSiBodyRidp = new unsigned[MapNdeformstruc]();		MemCpuFixed += (sizeof(unsigned) * MapNdeformstruc);
		DSParent = new unsigned[MapNdeformstruc]();			MemCpuFixed += (sizeof(unsigned) * MapNdeformstruc);
		DSPos0 = new tdouble3[MapNdeformstruc];				MemCpuFixed += (sizeof(tdouble3) * MapNdeformstruc);
		DSCode = new typecode[MapNdeformstruc];				MemCpuFixed += (sizeof(typecode) * MapNdeformstruc);
		DSPartFBC = (tbcstruc*)malloc(sizeof(tbcstruc) * MapNdeformstruc); MemCpuFixed += (sizeof(tbcstruc) * MapNdeformstruc);
		DSPartVBC = (tbcstruc*)malloc(sizeof(tbcstruc) * MapNdeformstruc); MemCpuFixed += (sizeof(tbcstruc) * MapNdeformstruc);
		// Only allocate phi BC array if phi BCs are actually defined
		if (DeformStruc && DeformStruc->HasPhiBC()) {
			DSPartPhiBC = (tphibc*)malloc(sizeof(tphibc) * MapNdeformstruc); MemCpuFixed += (sizeof(tphibc) * MapNdeformstruc);
		}
		else {
			DSPartPhiBC = NULL;  // Not needed
		}
		DSDisp = new tdouble3[MapNdeformstruc];				MemCpuFixed += (sizeof(tdouble3) * MapNdeformstruc); std::fill_n(DSDisp, MapNdeformstruc, TDouble3(0.0, 0.0, 0.0));
		DSVel = new tdouble3[MapNdeformstruc];				MemCpuFixed += (sizeof(tdouble3) * MapNdeformstruc);
		if(TStep==STEP_Symplectic) DSVelPre = new tdouble3[MapNdeformstruc];	MemCpuFixed += (sizeof(tdouble3) * MapNdeformstruc);
		DSAccl = new tdouble3[MapNdeformstruc];				MemCpuFixed += (sizeof(tdouble3) * MapNdeformstruc);
		DSFlForce = new tdouble3[MapNdeformstruc];			MemCpuFixed += (sizeof(tdouble3) * MapNdeformstruc);
		DSPairStart = new unsigned[MapNdeformstruc]();		MemCpuFixed += (sizeof(unsigned) * MapNdeformstruc);
		DSPairN = new unsigned[MapNdeformstruc]();			MemCpuFixed += (sizeof(unsigned) * MapNdeformstruc);
		DSDcellc = new unsigned[MapNdeformstruc]();			MemCpuFixed += (sizeof(unsigned) * MapNdeformstruc);
		DSCorrArray = new tdouble3[MapNdeformstruc];		MemCpuFixed += (sizeof(tdouble3) * MapNdeformstruc);
		DSDefGrad = new tmatrix3d[MapNdeformstruc];			MemCpuFixed += (sizeof(tmatrix3d) * MapNdeformstruc);
		DSJacob = new double[MapNdeformstruc]();			MemCpuFixed += (sizeof(double) * MapNdeformstruc);
		DSPiolKir = new tmatrix3d[MapNdeformstruc];			MemCpuFixed += (sizeof(tmatrix3d) * MapNdeformstruc);
		DSKerSumVol = new float[MapNdeformstruc]();		MemCpuFixed += (sizeof(float) * MapNdeformstruc);
		if (DSPlastic) {
			DSEqPlastic = new double[MapNdeformstruc]();	MemCpuFixed += (sizeof(double) * MapNdeformstruc);
			std::fill(DSEqPlastic, DSEqPlastic + MapNdeformstruc, 0.0);
			DSPlasticStrain = new tmatrix3d[MapNdeformstruc]; MemCpuFixed += (sizeof(tmatrix3d) * MapNdeformstruc);
			std::fill(DSPlasticStrain, DSPlasticStrain + MapNdeformstruc, TMatrix3d(0.0));
		}
		if (DSFracture) {
			DSPhi = new double[MapNdeformstruc]();				MemCpuFixed += (sizeof(double) * MapNdeformstruc); std::fill(DSPhi, DSPhi + MapNdeformstruc, 1.0);
			DSPhiDot = new double[MapNdeformstruc]();			MemCpuFixed += (sizeof(double) * MapNdeformstruc); std::fill(DSPhiDot, DSPhiDot + MapNdeformstruc, 0.0);
			DSPhiDDot = new double[MapNdeformstruc]();			MemCpuFixed += (sizeof(double) * MapNdeformstruc); std::fill(DSPhiDDot, DSPhiDDot + MapNdeformstruc, 0.0);
			DSHis = new double[MapNdeformstruc]();				MemCpuFixed += (sizeof(double) * MapNdeformstruc); std::fill(DSHis, DSHis + MapNdeformstruc, 0.0);
			if (TStep == STEP_Symplectic) {
				DSPhiDotPre = new double[MapNdeformstruc]();			MemCpuFixed += (sizeof(double) * MapNdeformstruc); std::fill(DSPhiDotPre, DSPhiDotPre + MapNdeformstruc, 0.0);
			}
		}
	}
}
...
void JSphCpu::DSUpdate_OrgVelPos()const {
	int cnd = static_cast<int>(CaseNdeformstruc);
#ifdef OMP_USE
#pragma omp parallel for if (cnd > OMP_LIMIT_COMPUTESTEPDEFSTRUC)
#endif
	for (int p = 0; p < cnd; p++) {
		const unsigned pds = DeformStrucRidp[p];
		const unsigned pdmap = DSBestChild[p];
		Posc[pds] = DSPosOrg0[p] + DSDisp[pdmap];
		Velrhopc[pds] = TFloat4(float(DSVel[pdmap].x), float(DSVel[pdmap].y), float(DSVel[pdmap].z), Velrhopc[pds].w);
	}
}

void JSphCpu::DSApplyFluidPressure(const double dt) {
	
#ifdef OMP_USE
#pragma omp parallel for if(DSNpSurf>OMP_LIMIT_COMPUTESTEPDEFSTRUC)
#endif
	for (int p = 0; p < DSNpSurf; p++) {
		const unsigned p1 = DSSurfPartList[p];
		tdouble3 pos0p1 = DSPos0[p1];
		tdouble3 dispp1 = DSDisp[p1];
		const tdouble3 posp1 = pos0p1 + dispp1;
		const unsigned pairnp1 = DSPairN[p1];
		const unsigned pairadd = DSPairStart[p1];
		const unsigned bodyid = CODE_GetIbodyDeformStruc(DSCode[p1]);
		StDeformStrucIntData body = DefStrucIntData[bodyid];
		tdouble3 acclp1 = TDouble3(0.0);
		tdouble3 normp1 = TDouble3(0.0);
		for (unsigned pp2 = 0; pp2 < pairnp1; pp2++) {
			const unsigned p2 = DSPairJ[pp2 + pairadd];
			const tdouble3 posp2 = DSPos0[p2] + DSDisp[p2];
			normp1 -= (posp2 - posp1) * DSKer[pp2 + pairadd];
		}
		const double normmag = fmath::DotVec3(normp1, normp1);
		if (normmag > ALMOSTZERO) normp1 = normp1 / sqrt(normmag);
		//tmatrix3d defg = DSDefGrad[p1];
		//tmatrix3d cauchy = 1.0 / fmath::Determinant3x3(defg) * fmath::MulMatrix3x3(defg, DSPiolKir[p1]);
		//double pressp1 = Simulate2D ? (cauchy.a11 + cauchy.a33) / 2.0 : (cauchy.a11 + cauchy.a22 + cauchy.a33) / 3.0;
		//
		double pressav = 0.0;
		double pressavsum = 0.0;
		const StNgSearch ngs = nsearch::Init(posp1, false, DivData); 
		for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
			const tuint2 pif = nsearch::ParticleRange(y, z, ngs, DivData);
			for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
				if (CODE_IsFluid(Codec[p2])) {
					const tdouble3 posp2 = Posc[p2];
					const float drx = float(posp1.x - float(posp2.x));
					const float dry = float(posp1.y - float(posp2.y));
					const float drz = float(posp1.z - float(posp2.z));
					const float rr2 = drx * drx + dry * dry + drz * drz;
					if (rr2 <= KernelSize2 && rr2 >= ALMOSTZERO) {
						//const float densp2 = Velrhopc[p2].w;
						//const float fac = fsph::GetKernel_Fac<KERNEL_Wendland>(CSP, rr2);
						const float fac = fsph::GetKernel_Wab<KERNEL_Wendland>(CSP, rr2);
						const float presp2 = Pressc[p2];
						if(presp2>0.0){
							pressav -= presp2 * fac;
							pressavsum += fac;
						}
						//const float frx = fac * drx, fry = fac * dry, frz = fac * drz;
						//const float prs = float(Pressc[p2] / (densp2 * densp2));// +pressp1 / (body.rho0 * body.rho0));
						//const float p_vpm = -prs * MassFluid;
						//acclp1.x += p_vpm * frx; acclp1.y += p_vpm * fry; acclp1.z += p_vpm * frz;
					}
				}

			}
		}
		if (pressavsum > ALMOSTZERO) pressav = pressav / pressavsum;
		float tobforce = 1.0f;

		//tobforce *= float((DSKerSumVol[p1] / body.vol0));
		//if (Simulate2D) tobforce *= 1.0f / float(massp * sqrt(body.vol0));
		//else tobforce *= 1.0f / float((massp * cbrt(body.vol0)));
		tobforce *= 1.0f / float((body.rho0));
		DSFlForce[p1] = pressav * normp1 * tobforce;
	}
}
...
#pragma omp parallel for schedule (static) if(npf>OMP_LIMIT_COMPUTESTEP)
	...
	//	ComputeSymplecticCorrDeformStruc(dt05, dt, Posc, Dcellc, Codec);
	//	BoundChanged = true;
...
double JSphCpu::DtVariable(bool final) {
	...
	if (final) {
		...
		if (SaveDt)SaveDt->AddValues(TimeStep, dt, dt1 * CFLnumber, dt2 * CFLnumber, DSMaxDt*CFLnumber, AceMax, ViscDtMax, DSMaxDt, VelMax);
	}
	return(dt);
}
...
//========================================================================================
/// Calculates total number of pairs for Total lagrangian kernels in all deformable structures 
//========================================================================================
unsigned JSphCpu::DSCountTotalPairs(StDeformStrucData* defstrucdatac)
{
	plane4Nstruc notchlist[MAX_NOTCH_DEFSTRUC];
	const double factorrr = Simulate2D ? 1.42 : 1.74;
#ifdef OMP_USE
#pragma omp parallel for schedule (static) if(MapNdeformstruc>OMP_LIMIT_COMPUTESTEPDEFSTRUC) private(notchlist)
#endif
	for (int pid = 0; pid < MapNdeformstruc; pid++) {
		tdouble3 posp1 = DSPos0[pid];
		DSPairN[pid] = 0;
		const unsigned bodyid = CODE_GetIbodyDeformStruc(DSCode[pid]);
		StDeformStrucData& body = defstrucdatac[bodyid];
		const float ksize2 = body.kernelsize2;
		const double distfact = factorrr * body.nbsrange;
		const float dp201 = static_cast<float>(distfact * body.dp * distfact * body.dp);
		const unsigned notchnm = body.nnotch;
		if (notchnm) std::copy(body.notchlist, body.notchlist + notchnm, std::begin(notchlist));

		const StNgSearch ngs = nsearch::InitDefSolid(posp1, &DSDivData);
		...
		for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
			...
			for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
				if (p2 == pid) continue;
				if (CODE_GetIbodyDeformStruc(DSCode[p2]) != bodyid) continue;

				const tdouble3 posp2 = DSPos0[p2];

				const float drxw = nsearch::Distance2(posp1, posp2);
	...
	for (int pid = 0; pid < MapNdeformstruc; pid++) {
		if (pid == 0) continue;
		DSPairStart[pid] = DSPairStart[pid - 1] + DSPairN[pid - 1];
	}

	return DSPairStart[MapNdeformstruc - 1] + DSPairN[MapNdeformstruc - 1];
}

//========================================================================================
/// Checks whether the line p1p2 intersects with any of the 4 noded surface in the list rect4n
/// p1: point 1 in 3D
/// P2: point 2 in 3D
/// rect4n List of surfaces each enclosed by 4 nodes
/// size4n size of rect4n
/// Returns true if intersection occurs between p1p2 and any of the 4 noded surfaces in the list rect4n
//========================================================================================
bool JSphCpu::DSLineSurfIntersect(const tdouble3 part1, const tdouble3 part2, const plane4Nstruc* rect4n, const unsigned size4n)const {

	tdouble3 domainAmin, domainAmax, domainBmin, domainBmax;
	tdouble3 AdjuncVec1, AdjuncVec2, AdjuncVec3;
	tdouble3 LineVec1, LineVec2, LineVec3, xint, tVec1;
	...
	domainAmin = { min(part1.x, part2.x), min(part1.y, part2.y), min(part1.z, part2.z) };
	domainAmax = { max(part1.x, part2.x), max(part1.y, part2.y), max(part1.z, part2.z) };

	for (unsigned surfi = 0; surfi < size4n; surfi++) {

		LineVec1 = rect4n[surfi].corners[1] - rect4n[surfi].corners[0];
		LineVec2 = rect4n[surfi].corners[2] - rect4n[surfi].corners[0];
		LineVec3 = rect4n[surfi].corners[3] - rect4n[surfi].corners[0];

		domainBmin = { 1.0e20, 1.0e20, 1.0e20 };
		domainBmax = { -1.0e20, -1.0e20, -1.0e20 };

		for (int i = 0; i < 4; ++i) {
			domainBmin.x = min(domainBmin.x, rect4n[surfi].corners[i].x);
			domainBmin.y = min(domainBmin.y, rect4n[surfi].corners[i].y);
			domainBmin.z = min(domainBmin.z, rect4n[surfi].corners[i].z);
			domainBmax.x = max(domainBmax.x, rect4n[surfi].corners[i].x);
			domainBmax.y = max(domainBmax.y, rect4n[surfi].corners[i].y);
			domainBmax.z = max(domainBmax.z, rect4n[surfi].corners[i].z);
		}
		...
		tVec1 = part1 - rect4n[surfi].corners[0];
...
void JSphCpu::DSCalcKers_ct(StDeformStrucData* defstrucdatac)const
{
	plane4Nstruc notchlist[MAX_NOTCH_DEFSTRUC];
	double dppp = 0.0;
	const double factorrr = Simulate2D ? 1.42 : 1.74;
#ifdef OMP_USE
#pragma omp parallel for if(MapNdeformstruc>OMP_LIMIT_COMPUTESTEPDEFSTRUC) private(notchlist)
#endif
	for (int pid = 0; pid < MapNdeformstruc; pid++) {
		tdouble3 posp1 = DSPos0[pid];
		DSPairN[pid] = 0;
		unsigned bodyid = CODE_GetIbodyDeformStruc(DSCode[pid]);
		StDeformStrucData& body = defstrucdatac[bodyid];
		const float ksize2 = body.kernelsize2; 
		const double distfact = factorrr * body.nbsrange;
		const float dp201 = static_cast<float>(distfact * body.dp * distfact * body.dp);
		const unsigned notchnm = body.nnotch;
		const float vol0p1 = body.vol0;
		const float bkernh = body.kernelh;
		if (notchnm) std::copy(body.notchlist, body.notchlist + notchnm, std::begin(notchlist));
		//-Search for neighbours in adjacent cells.
		const StNgSearch ngs = nsearch::InitDefSolid(posp1, &DSDivData);
		const unsigned pairstarti = DSPairStart[pid];
		double kersum = 0.0;
		for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
			const tuint2 pif = nsearch::DSParticleRange(y, z, &ngs, &DSDivData);
			...
			for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
				...
				const tdouble3 posp2 = DSPos0[p2];
				const tdouble3 drx = posp1 - posp2;
				const float rr20 = fmath::DotVecf3(drx, drx);
				if (rr20 > ksize2) continue;
				if (rr20 > dp201) continue;
				if (notchnm && DSLineSurfIntersect(posp1, posp2, notchlist, notchnm)) continue;
				//-Computes kernel.
				float fac0 = 0;
				const unsigned pairid = pairstarti + DSPairN[pid];
				
				DSKer[pairid] = fsph::GetTLSPHKernel_WabFac<tker>(Simulate2D, bkernh, rr20, fac0); //kernel
				DSKerDerV0[pairid] = fac0 * drx;
				DSPairJ[pairid] = p2;
				kersum += DSKer[pairid];
				DSPairN[pid]++;
			}
		...
		kersum +=fsph::GetTLSPHKernel_Wab<tker>(Simulate2D, bkernh, 0.0);
		DSKerSumVol[pid] = float(1.0f / kersum);
		if (DSPairN[pid]) {
			tmatrix3d kercorrp1 = { 0 };
			const unsigned pairnum = DSPairN[pid];

			for (unsigned np = 0; np < pairnum; np++) kercorrp1 += fmath::DyadicVec3(DSKerDerV0[pairstarti + np], (DSPos0[DSPairJ[pairstarti + np]] - posp1));

			tmatrix3d kercorr = (Simulate2D ? fmath::InverseMatrix2x2(kercorrp1) : fmath::InverseMatrix3x3(kercorrp1));

			for (unsigned np = 0; np < pairnum; np++) {
				const unsigned pairid = pairstarti + np;
				//Correct kernels and their first derivatives
				DSKer[pairid] = DSKer[pairid] / kersum;
				DSKerDerV0[pairid] = fmath::MulMatrix3x3Vec3(kercorr, DSKerDerV0[pairid]);

				//Laplacian of corrected kernels
				const tdouble3 drx = DSPos0[DSPairJ[pairid]] - posp1;
				const float rr20 = fmath::DotVecf3(drx, drx);
				DSKerLapc[pairid] = fmath::DotVec3(drx, DSKerDerV0[pairid]) * 2.0 / rr20;
			}
		}
	}
}

void JSphCpu::DSCalcKers(StDeformStrucData* defstrucdatac)const {
	if (TKernel == KERNEL_Cubic) DSCalcKers_ct<KERNEL_Cubic>(defstrucdatac);
	else DSCalcKers_ct<KERNEL_Wendland>(defstrucdatac);
}

float JSphCpu::DSGetSelfKernBody(float bkernh)const {
	if (TKernel == KERNEL_Cubic) return fsph::GetTLSPHKernel_Wab<KERNEL_Cubic>(Simulate2D, bkernh, 0.0);
	else return fsph::GetTLSPHKernel_Wab<KERNEL_Cubic>(Simulate2D, bkernh, 0.0);
}
