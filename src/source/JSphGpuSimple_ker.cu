...
#include "FunctionsBasic_iker.h"
	...
	//==============================================================================
	__global__ void KerComputeStepDeformStrucSemiImplicitEuler(unsigned n, const float4* velrhop, const typecode* code, const unsigned* deformstrucridp
		, const float3* ace, double dt, float3 gravity
		, double2* movxy, double* movz, float4* velrhopnew)
	{
		unsigned p = blockIdx.x * blockDim.x + threadIdx.x; //-Number of particle.
		if (p < n) {
			const unsigned p1 = deformstrucridp[p]; //-Number of particle.
			if (CODE_IsDeformStrucDeform(code[p1])) {
	...
	/// Actualiza la posición y la velocidad usando el esquema de Euler semiimplícito (usado con el esquema de Verlet).
	//==============================================================================
	void ComputeStepDeformStrucSemiImplicitEuler(unsigned npfs, const float4* velrhop, const typecode* code, const unsigned* deformstrucridp
		...
		, double2* movxy, double* movz, float4* velrhopnew, cudaStream_t stm)
	{
		if (npfs) {
			...
			KerComputeStepDeformStrucSemiImplicitEuler << <sgrid, SPHBSIZE, 0, stm >> > (npfs, velrhop, code, deformstrucridp
	...
	//==============================================================================
	__global__ void KerComputeStepDeformStrucSymplecticPre(unsigned n, const float4* velrhoppre, const typecode* code, const unsigned* deformstrucridp
	...
	/// Actualiza la posición y la velocidad utilizando un esquema predictor simpléctico.
	//==============================================================================
	void ComputeStepDeformStrucSymplecticPre(unsigned npfs, const float4* velrhoppre, const typecode* code, const unsigned* deformstrucridp
		, const float3* ace, double dtm, tfloat3 gravity
		, double2* movxy, double* movz, float4* velrhop, cudaStream_t stm)
	{
		if (npfs) {
			dim3 sgrid = GetSimpleGridSize(npfs, SPHBSIZE);
			KerComputeStepDeformStrucSymplecticPre << <sgrid, SPHBSIZE, 0, stm >> > (npfs, velrhoppre, code, deformstrucridp
	...
	//==============================================================================
	__global__ void KerComputeStepDeformStrucSymplecticCor(unsigned n, const float4* velrhoppre, const typecode* code, const unsigned* deformstrucridp
	...
	/// Actualiza la posición y la velocidad utilizando un esquema corrector simpléctico.
	//==============================================================================
	void ComputeStepDeformStrucSymplecticCor(unsigned npfs, const float4* velrhoppre, const typecode* code, const unsigned* deformstrucridp
		, const float3* ace, double dtm, double dt, tfloat3 gravity
		, double2* movxy, double* movz, float4* velrhop, cudaStream_t stm)
	{
		if (npfs) {
			dim3 sgrid = GetSimpleGridSize(npfs, SPHBSIZE);
			KerComputeStepDeformStrucSymplecticCor << <sgrid, SPHBSIZE, 0, stm >> > (npfs, velrhoppre, code, deformstrucridp
