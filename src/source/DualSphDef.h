...
#define SIGN_PLASTIC_FLOW 1e-1f
#define SKIP_SENTINELg -3.402823466e+38f
#define MAX_VELBC_DEFSTRUC 512		///<Maximum number of velocity boundary conditions on a deformable structure
#define MAX_FORCEBC_DEFSTRUC 512		///<Maximum number of force boundary conditions on a deformable structure
#define MAX_PHIBC_DEFSTRUC 512		///<Maximum number of phi boundary conditions on a deformable structure
#define MAX_NOTCH_DEFSTRUC 512       ///<Maximum number of preexisting notches on a deformable structure
#define MAX_MEASUREPLANE_DEFSTRUC 512       ///<Maximum number of measuring plane on a deformable structure

#define DSBC_X_FLAG           ((uint64_t)1 << 0)
#define DSBC_Y_FLAG           ((uint64_t)1 << 1)
#define DSBC_Z_FLAG           ((uint64_t)1 << 2)
#define DSBC_X_IS_EXPR        ((uint64_t)1 << 3)
#define DSBC_Y_IS_EXPR        ((uint64_t)1 << 4)
#define DSBC_Z_IS_EXPR        ((uint64_t)1 << 5)

#define DSBC_X_EXPRID_SHIFT   6
#define DSBC_X_EXPRID_MASK    ((uint64_t)0xFFFF << DSBC_X_EXPRID_SHIFT)
#define DSBC_Y_EXPRID_SHIFT   22
#define DSBC_Y_EXPRID_MASK    ((uint64_t)0xFFFF << DSBC_Y_EXPRID_SHIFT)
#define DSBC_Z_EXPRID_SHIFT   38
#define DSBC_Z_EXPRID_MASK    ((uint64_t)0xFFFF << DSBC_Z_EXPRID_SHIFT)
#define DSBC_SURFACE_FLAG     ((uint64_t)1 << 54)


#define DSBC_FORCETYPE_SHIFT  55
#define DSBC_FORCETYPE_MASK   ((uint64_t)0x3 << DSBC_FORCETYPE_SHIFT)
#define DSBC_FORCETYPE_SURFACE  0  // Surface force: N/m^2 (3D) or N/m (2D)
#define DSBC_FORCETYPE_BODY     1  // Body force: N/m^3 (3D) or N/m^2 (2D)
#define DSBC_FORCETYPE_POINT    2  // Point force: N (Newtons)

// Phi BC flag definitions (using x component for expression ID and restore value)
#define DSBC_PHI_ACTIVE       ((uint64_t)1 << 57)  // Phi BC is active
#define DSBC_PHI_EXPRID_SHIFT DSBC_X_EXPRID_SHIFT  // Reuse X shift for expression ID
#define DSBC_PHI_EXPRID_MASK  DSBC_X_EXPRID_MASK   // Reuse X mask for expression ID

#define DSBC_SET_X_EXPRID(f, v) do { (f) = ((f) & ~DSBC_X_EXPRID_MASK) | (((uint64_t)(v) & 0xFFFF) << DSBC_X_EXPRID_SHIFT); } while (0)
#define DSBC_SET_Y_EXPRID(f, v) do { (f) = ((f) & ~DSBC_Y_EXPRID_MASK) | (((uint64_t)(v) & 0xFFFF) << DSBC_Y_EXPRID_SHIFT); } while (0)
#define DSBC_SET_Z_EXPRID(f, v) do { (f) = ((f) & ~DSBC_Z_EXPRID_MASK) | (((uint64_t)(v) & 0xFFFF) << DSBC_Z_EXPRID_SHIFT); } while (0)
#define DSBC_GET_X_EXPRID(f) ((unsigned int)(((f) & DSBC_X_EXPRID_MASK) >> DSBC_X_EXPRID_SHIFT))
#define DSBC_GET_Y_EXPRID(f) ((unsigned int)(((f) & DSBC_Y_EXPRID_MASK) >> DSBC_Y_EXPRID_SHIFT))
#define DSBC_GET_Z_EXPRID(f) ((unsigned int)(((f) & DSBC_Z_EXPRID_MASK) >> DSBC_Z_EXPRID_SHIFT))
#define DSBC_SET_X_FLAG(f,val)     do { if(val) (f)|=DSBC_X_FLAG; else (f)&=~DSBC_X_FLAG; } while(0)
#define DSBC_GET_X_FLAG(f)         (((f)&DSBC_X_FLAG)!=0)
#define DSBC_SET_Y_FLAG(f,val)     do { if(val) (f)|=DSBC_Y_FLAG; else (f)&=~DSBC_Y_FLAG; } while(0)
#define DSBC_GET_Y_FLAG(f)         (((f)&DSBC_Y_FLAG)!=0)
#define DSBC_SET_Z_FLAG(f,val)     do { if(val) (f)|=DSBC_Z_FLAG; else (f)&=~DSBC_Z_FLAG; } while(0)
#define DSBC_GET_Z_FLAG(f)         (((f)&DSBC_Z_FLAG)!=0)
#define DSBC_SET_X_IS_EXPR(f,val)  do { if(val) (f)|=DSBC_X_IS_EXPR; else (f)&=~DSBC_X_IS_EXPR; } while(0)
#define DSBC_GET_X_IS_EXPR(f)      (((f)&DSBC_X_IS_EXPR)!=0)
#define DSBC_SET_Y_IS_EXPR(f,val)  do { if(val) (f)|=DSBC_Y_IS_EXPR; else (f)&=~DSBC_Y_IS_EXPR; } while(0)
#define DSBC_GET_Y_IS_EXPR(f)      (((f)&DSBC_Y_IS_EXPR)!=0)
#define DSBC_SET_Z_IS_EXPR(f,val)  do { if(val) (f)|=DSBC_Z_IS_EXPR; else (f)&=~DSBC_Z_IS_EXPR; } while(0)
#define DSBC_GET_Z_IS_EXPR(f)      (((f)&DSBC_Z_IS_EXPR)!=0)
#define DSBC_SET_SURFACE(f,val)    do { if(val) (f)|=DSBC_SURFACE_FLAG; else (f)&=~DSBC_SURFACE_FLAG; } while(0)
#define DSBC_GET_SURFACE(f)        (((f)&DSBC_SURFACE_FLAG)!=0)

// Force type macros
#define DSBC_SET_FORCETYPE(f,type) do { (f) = ((f) & ~DSBC_FORCETYPE_MASK) | (((uint64_t)(type) & 0x3) << DSBC_FORCETYPE_SHIFT); } while(0)
#define DSBC_GET_FORCETYPE(f)      ((unsigned int)(((f) & DSBC_FORCETYPE_MASK) >> DSBC_FORCETYPE_SHIFT))

// Phi BC macros
#define DSBC_SET_PHI_ACTIVE(f,val)    do { if(val) (f)|=DSBC_PHI_ACTIVE; else (f)&=~DSBC_PHI_ACTIVE; } while(0)
#define DSBC_GET_PHI_ACTIVE(f)        (((f)&DSBC_PHI_ACTIVE)!=0)
#define DSBC_SET_PHI_EXPRID(f, v)     DSBC_SET_X_EXPRID(f, v)
#define DSBC_GET_PHI_EXPRID(f)        DSBC_GET_X_EXPRID(f)
...
//<vs_deformstruc_ini>
#define CODE_TYPE_DEFORMSTRUC_MASK 0x07e0         //-Bits for deformable structure: 0000 0111 1110 0000
//<vs_deformstruc_end>
...
//<vs_deformstruc_ini>
#define MAX_DEFORMSTRUC_NUM 16
#define MAX_DEFSTRUC_USREXP_NUM 1024
#define EXP_MASK_ID       0x07FF
#define EXP_SHIFT_ID      0
#define EXP_MAX_INDEX     2048
#define GeExpressIndex(id) ((id & EXP_MASK_ID) >> EXP_SHIFT_ID)

#define CODE_IsDeformStrucAny(code)   ((CODE_IsFixed(code)||CODE_IsMoving(code)) && (code&CODE_TYPE_DEFORMSTRUC_MASK)==CODE_TYPE_DEFORMSTRUC_MASK)
#define CODE_IsDeformStrucDeform(code)  (CODE_IsMoving(code) && (code&(CODE_TYPE_DEFORMSTRUC_MASK))==CODE_TYPE_DEFORMSTRUC_MASK)

#define CODE_ToDeformStrucDeform(code,ibody)  ((code&(CODE_MASKSPECIAL|CODE_MASKTYPE))|CODE_TYPE_DEFORMSTRUC_MASK|ibody)
#define CODE_GetIbodyDeformStruc(code)      (code&(~(CODE_MASKSPECIAL|CODE_MASKTYPE|CODE_TYPE_DEFORMSTRUC_MASK)))
//<vs_deformstruc_end>
...
//<vs_deformstruc_ini>
typedef enum {
	CONSTITMODEL_SVK = 1,           ///<St. Venant Kirchhoff.
	CONSTITMODEL_NH = 2,            ///<Neo-Hookean hyperelastic.
	CONSTITMODEL_J2 = 3             ///<J2 elasto-plastic with isotropic hardening.
}TpConstitModel;
///Structure with the information of the deformable structure.
typedef struct {
	...
	typecode mkbound;						///<Mkbound information of the structure
	float avfactor1;							///<Artificial viscousity correction factor1.
	float avfactor2;							///<Artificial viscousity correction factor2.


	unsigned mapfact;						///<Map factor for particle discretization.
	tbcstrucbody bcvel[MAX_VELBC_DEFSTRUC];						///<List of velocity boundary conditions [m/s].
	tbcstrucbody bcforce[MAX_FORCEBC_DEFSTRUC];					///<List of force boundary conditions [surface: N/m^2(3D)|N/m(2D), body: N/m^3(3D)|N/m^2(2D), point: N].
	tbcstrucbody bcphi[MAX_PHIBC_DEFSTRUC];						///<List of phi boundary conditions with expression ID and restore value.
	plane4Nstruc notchlist[MAX_NOTCH_DEFSTRUC];					///<List of preexisint notches in the structure.
	plane4Nstruc measplanelist[MAX_MEASUREPLANE_DEFSTRUC];		///<List of measuring planes in the structure.
	...
	unsigned nnotch;								///<Number of preexisitng notches in the structure
	unsigned nmeasplane;							///<Number of measuring planes in the structure
	tdouble3 min;									///<Minimum bounds of the domain
	tdouble3 max;									///<Maximum bounds of the domain
	float tau;								///<Value of (1-poisson^2)/young (units:-).
	float mass;								///<Total Mass of body
	...
	float lamelambda;						///<Lame parameter, Lambda
	float lamemu;							///<Lame parameter, Mu
	float lamebulk;							///<Bulk modulus
	float czero;							///<Initial speed of sound
	float kernelsize;						///<Maximum interaction distance between particles (KernelK*KernelH) of the deform struc body.
	float kernelsize2;						///<Maximum interaction distance squared (KernelSize^2) of the deform struc body.
	float kernelh;							///<The smoothing length of SPH kernel [m] for each deform struc body.
	float selfkern;							///<value of sph kernel at 0
	unsigned npbody;						///<Number of generated particle for the body
	unsigned npstart;						///<Starting index of particles for a body
	unsigned nbsrange;						///<Restrict nieghbour search to nbsrange number of particle
}StDeformStrucData;

///Structure with the information of the deformable structure for time integration.
typedef struct StDeformStrucIntData {
	typecode mkbound;
	unsigned npbody;				///<Number of generated particle for the body
	unsigned npstart;				///<Starting index of particles for a body
	float pflim;					//Phase field soft particle limit
	bool fracture;					//body fractures
	float lambda;					//Lame parameter lambda
	float bulk;						//Bulk modulus
	float young;					//Young's modulus
	float mu;						//Shear modulus
	float vol0;						//Volume of particle
	float rho0;						//Density
	float czero;					//Speed of sound
	float gc;						//Critical energy release rate
	float lc;						//Phase field length scale
	float avfactor1;				//Artificial viscousity factor1
	float avfactor2;				//Artificial viscousity factor2
	unsigned nbsrange;				///<Restrict nieghbour search to nbsrange number of particle
	float kernelh;					//Kernel h
	float tau;						///<Value of (1-poisson^2)/young (units:-).
	float mass;						///<Total Mass of body
	int mapfact;					///<Map Factor
	float dp;						///<Body dp
	float restcoeff;				///<Coefficient of restitution
	float kfric;					///<Coefficient of kinetic friction
	float yieldstress;                      ///<Initial yield stress for J2 plasticity
	...
	float selfkern;							///<value of sph kernel at 0
	TpConstitModel constitmodel;	//Constitutive model
	unsigned nmeasplane;			///<Starting index of particles for a body

	StDeformStrucIntData(const unsigned _npbody, const unsigned _npstart, const typecode _mkbound, const float _pflim, const bool _fracture, const float _lambda,
		const float _bulk, const float _mu, const float _vol0, const float _rho0,
		const float _czero, const float _gc, const float _lc, const float _avfactor1, const float _avfactor2,
		const float _mass, const float _kfric, const float _yieldstress, const float _hardening, const float _tau, const float _restcoeff, const int _mapfac,
		const float _kernelh, const TpConstitModel _constitmodel, const unsigned _nmeasplane, const float _dp, const float _young, const unsigned nbsrange_, float selfkern_) :
		pflim(_pflim), fracture(_fracture), lambda(_lambda), bulk(_bulk), mu(_mu),
		vol0(_vol0), rho0(_rho0), czero(_czero), gc(_gc), lc(_lc), avfactor1(_avfactor1), avfactor2(_avfactor2),
		kernelh(_kernelh), constitmodel(_constitmodel), npbody(_npbody), npstart(_npstart),
		mkbound(_mkbound), mass(_mass), kfric(_kfric), yieldstress(_yieldstress), hardening(_hardening), tau(_tau), restcoeff(_restcoeff), mapfact(_mapfac),
		nmeasplane(_nmeasplane), dp(_dp), young(_young), nbsrange(nbsrange_), selfkern(selfkern_)
	{
	}

} StDeformStrucIntData;
