...
//:# - Ahora GetAttributeUnsigned permiten todo el rango de unsigned. (06-05-2016)
...
public:
  ...
  /// \param name Element name to look for.
	...
	/// \param valdef Value by default if it does not exist and \a optional was activated. 
	/// \throw JException The requested attribute does not exist...
	/// Prints the name of the attribute in the exception.
	//==============================================================================
	unsigned GetAttributeUnsignedn(const TiXmlElement* ele, const std::string& name
		, const std::string& attr, bool optional = false, unsigned valdef = 0)const;
	...
	//==============================================================================
	/// Checks and returns a value of type double positive of an xml element. 
	/// \param ele Xml element.
	/// \param name Name of the requested attribute.
	/// \param optional If it does not exist,
	/// returns \a valdef instead of throwing an exception.
	...
	/// \throw JException The requested attribute does not exist...
	//==============================================================================
	double GetAttributeDoublePos(const TiXmlElement* ele, const std::string& name, const std::string& attr
		, bool optional = false, double valdef = 0)const;
	//==============================================================================
	...
	//==============================================================================
	bool CheckAttribute(const TiXmlElement* ele, const std::string& name)const;
	//==============================================================================
	...
	tuint3 GetAttributeUint3(const TiXmlElement* ele)const {
		return(TUint3(GetAttributeUnsigned(ele, "x")
			, GetAttributeUnsigned(ele, "y"), GetAttributeUnsigned(ele, "z")));
	}
	...
	//==============================================================================
	/// Checks and returns any component of type double3 of the xml element.
	/// \param ele Xml element to read.
	/// \param name1 Name of the first attribute (x by default).
	/// \param name2 Name of the second attribute (y by default).
	/// \param name3 Name of the third attribute (z by default).
	/// \throw JException Format not valid for the requested type...
	//==============================================================================
	tdouble3 GetAttributeDouble3any(const TiXmlElement* ele, const char* name1 = "x"
		, const char* name2 = "y", const char* name3 = "z")const
	{
		return(TDouble3(GetAttributeDouble(ele, name1,true), GetAttributeDouble(ele, name2, true)
			, GetAttributeDouble(ele, name3, true)));
	}

	//==============================================================================
	/// Checks and returns value of type bcstruc of the xml element.
	/// \param ele Xml element to read.
	/// \param name1 Name of the first attribute (x by default).
	/// \param name2 Name of the second attribute (y by default).
	/// \param name3 Name of the third attribute (z by default).
	/// \param name4 Name of the fourth attribute (mkid by default).
	/// \throw JException Format not valid for the requested type...
	//==============================================================================
	tbcstrucbody GetAttributeBCstruc(const TiXmlElement* ele)const
	{
		//, const char* name1 = "x", const char* name2 = "y", const char* name3 = "z", const char* name4 = "mkid"
		tbcstrucbody dsbc = make_tbcstrucbody0();
		
		DSBC_SET_SURFACE(dsbc.flags, CheckAttribute(ele, "mkid"));
		if (DSBC_GET_SURFACE(dsbc.flags)) {
			dsbc.mkid = GetAttributeWord(ele, "mkid");
		}

		if (ele && std::string(ele->Value()) == "bcforce") {
			unsigned forcetype = GetAttributeUnsigned(ele, "type", true, 4);
			if (forcetype == 4 && CheckAttribute(ele, "mkid")) forcetype = 2;
			if (forcetype == 2) {
				DSBC_SET_FORCETYPE(dsbc.flags, DSBC_FORCETYPE_SURFACE);
			}
			else if (forcetype == 3) {
				DSBC_SET_FORCETYPE(dsbc.flags, DSBC_FORCETYPE_BODY);
			}
			else if (forcetype == 1) {
				DSBC_SET_FORCETYPE(dsbc.flags, DSBC_FORCETYPE_POINT);
			}
			else {
				// Invalid forcetype value
				RunException("GetAttributeBCstruc", std::string("Invalid forcetype value '") + std::to_string(forcetype) +
					"'. Valid values are: '1' for point load, '2' for surface traction, '3' for body force");
			}
		}
		// Check for X: either "x" or "xe", not both
		bool hasX = CheckAttribute(ele, "x");
		bool hasXE = CheckAttribute(ele, "xe");
		if (hasX && hasXE) {
			// Handle error: both "x" and "xe" present
			// Example: Log error, throw exception, or prioritize one
			// Here, we'll prioritize the expression ("xe") as an example
			hasX = false;
		}
		DSBC_SET_X_FLAG(dsbc.flags, hasX);
		DSBC_SET_X_IS_EXPR(dsbc.flags, hasXE);

		// Similarly for Y and Z
		bool hasY = CheckAttribute(ele, "y");
		bool hasYE = CheckAttribute(ele, "ye");
		if (hasY && hasYE) {
			hasY = false;
		}
		DSBC_SET_Y_FLAG(dsbc.flags, hasY);
		DSBC_SET_Y_IS_EXPR(dsbc.flags, hasYE);

		bool hasZ = CheckAttribute(ele, "z");
		bool hasZE = CheckAttribute(ele, "ze");
		if (hasZ && hasZE) {
			hasZ = false;
		}
		DSBC_SET_Z_FLAG(dsbc.flags, hasZ);
		DSBC_SET_Z_IS_EXPR(dsbc.flags, hasZE);

		// Retrieve values if flags are set
		if (DSBC_GET_X_FLAG(dsbc.flags)) {
			dsbc.x = GetAttributeFloat(ele, "x");
		}
		if (DSBC_GET_Y_FLAG(dsbc.flags)) {
			dsbc.y = GetAttributeFloat(ele, "y");
		}
		if (DSBC_GET_Z_FLAG(dsbc.flags)) {
			dsbc.z = GetAttributeFloat(ele, "z");
		}

		// Set EXPRIDs if IS_EXPR is set
		if (DSBC_GET_X_IS_EXPR(dsbc.flags)) {
			DSBC_SET_X_EXPRID(dsbc.flags, GetAttributeUnsigned(ele, "xe"));
		}
		if (DSBC_GET_Y_IS_EXPR(dsbc.flags)) {
			DSBC_SET_Y_EXPRID(dsbc.flags, GetAttributeUnsigned(ele, "ye"));
		}
		if (DSBC_GET_Z_IS_EXPR(dsbc.flags)) {
			DSBC_SET_Z_EXPRID(dsbc.flags, GetAttributeUnsigned(ele, "ze"));
		}
		
		dsbc.tst = GetAttributeFloat(ele, "tstart", true, 0.0);
		dsbc.tend = GetAttributeFloat(ele, "tend", true, FLT_MAX);
		dsbc.tst -= ALMOSTZERO;
		if (dsbc.tend < 0.99 * FLT_MAX) dsbc.tend += ALMOSTZERO;
		return dsbc;
	}
	...
	//==============================================================================
	/// Calls \ref ReadElementDoublePos() with the same parameters.
	//==============================================================================
	float ReadElementFloatPos(const TiXmlNode* node, const std::string& name
		, const std::string& attrib, bool optional = false, float valdef = 0)const
	{
		return(float(ReadElementDoublePos(node, name, attrib, optional, valdef)));
	}
	//==============================================================================
	...
	//==============================================================================
	/// Checks and returns value of type double positive of the first xml element 
	/// of a node with a given name.
	/// \param name Name of the element to be reached.
	/// \param node Xml node where the element is reached.
	/// \param attrib Name of the requested attribute.
	...
	/// \throw JException Format not valid for the requested type...
	//==============================================================================
	double ReadElementDoublePos(const TiXmlNode* node, const std::string& name
		, const std::string& attrib, bool optional = false, double valdef = 0)const
	{
		TiXmlElement* ele = GetFirstElement(node, name, optional);
		return(ele ? GetAttributeDoublePos(ele, name, attrib, optional, valdef) : valdef);
	}
	...
	{
		TiXmlElement* ele = GetFirstElement(node, name, optional);
		return(ele ? GetAttributeUnsignedn(ele, name, attrib, optional, valdef) : valdef);
	}
	...
	//==============================================================================
	/// Loads a list of xml elements for boundary conditions of structures
	/// and returns the number of loaded elements.
	/// \param node Xml node the list of values is loaded from. 
	/// \param name Name of the elements of type tbcstruc.
	/// \param vec Array where loaded values are stored.
	/// \param count Maximum number of elements to load.
	/// \read NULL if Values missing or any value is not valid...
	//==============================================================================
	unsigned ReadArrayBCstruc(const TiXmlNode* node, const std::string& name
		, tbcstrucbody* vec, unsigned count) const;

	//==============================================================================
	/// Loads a list of xml elements for pre existing notches of structures
	/// and returns the number of loaded elements.
	/// \param node Xml node the list of values is loaded from. 
	/// \param name Name of the elements of type tnotchstruc.
	/// \param vec Array where loaded values are stored.
	/// \param count Maximum number of elements to load.
	/// \read NULL if Values missing or any value is not valid...
	//==============================================================================
	unsigned ReadArrayNotchstruc(const TiXmlNode* node, const std::string& name
		, plane4Nstruc* vec, unsigned count) const;

	//==============================================================================
	/// Checks and returns value of type notchstruc of the xml element.
	/// \param ele Xml element to read.
	/// \param name1 Name of the first attribute (x by default).
	/// \param name2 Name of the second attribute (y by default).
	/// \param name3 Name of the third attribute (z by default).
	...
	/// \throw JException Format not valid for the requested type...
	//==============================================================================
	plane4Nstruc GetAttributeNotchstruc(const TiXmlElement* node)const
	{
		plane4Nstruc notch = TNotchStrucNull();
		TiXmlElement* ele = GetFirstElement(node, "p1", false);
		notch.corners[0].x = GetAttributeFloat(ele, "x");
		notch.corners[0].y = GetAttributeFloat(ele, "y");
		notch.corners[0].z = GetAttributeFloat(ele, "z");

		ele = GetFirstElement(node, "p2", false);
		notch.corners[1].x = GetAttributeFloat(ele, "x");
		notch.corners[1].y = GetAttributeFloat(ele, "y");
		notch.corners[1].z = GetAttributeFloat(ele, "z");

		ele = GetFirstElement(node, "p3", false);
		notch.corners[2].x = GetAttributeFloat(ele, "x");
		notch.corners[2].y = GetAttributeFloat(ele, "y");
		notch.corners[2].z = GetAttributeFloat(ele, "z");

		ele = GetFirstElement(node, "p4", false);
		notch.corners[3].x = GetAttributeFloat(ele, "x");
		notch.corners[3].y = GetAttributeFloat(ele, "y");
		notch.corners[3].z = GetAttributeFloat(ele, "z");
		return notch;
	}
