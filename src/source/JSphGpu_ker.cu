...
#include "JCellSearch_iker.h"
		...
		, const unsigned* idp, const float3* boundnormal, const float3* motionvel, float4* velrhop)
	{
		switch (slipmode) {
		case SLIP_Vel0: {
			const TpSlipMode tslip = SLIP_Vel0;
		...
		case SLIP_NoSlip: {
			const TpSlipMode tslip = SLIP_NoSlip;
			if (simulate2d)Interaction_MdbcCorrectionT2 <tker, true, tslip>(fastsingle, n, nbound, mdbcthreshold, dvd, mapposmin, posxy, posz, poscell, code, idp, boundnormal, motionvel, velrhop);
			else          Interaction_MdbcCorrectionT2 <tker, false, tslip>(fastsingle, n, nbound, mdbcthreshold, dvd, mapposmin, posxy, posz, poscell, code, idp, boundnormal, motionvel, velrhop);
		}break;
		case SLIP_FreeSlip: {
			const TpSlipMode tslip = SLIP_FreeSlip;
			if (simulate2d)Interaction_MdbcCorrectionT2 <tker, true, tslip>(fastsingle, n, nbound, mdbcthreshold, dvd, mapposmin, posxy, posz, poscell, code, idp, boundnormal, motionvel, velrhop);
			else          Interaction_MdbcCorrectionT2 <tker, false, tslip>(fastsingle, n, nbound, mdbcthreshold, dvd, mapposmin, posxy, posz, poscell, code, idp, boundnormal, motionvel, velrhop);
		}break;
#endif
	...
	__global__ void DSKerResetForcesDem(unsigned MapNdeformstruc,  float3* DSFlForceg)
	{
		const unsigned p1 = blockIdx.x * blockDim.x + threadIdx.x;
		if (p1 < MapNdeformstruc) {
			DSFlForceg[p1] = make_float3(0, 0, 0);
		}
	}

	template <bool simulate2d>
	__global__ void DSKerInteractionForcesDem(unsigned MapNdeformstruc, unsigned bodycnt,
		...
		const unsigned* __restrict__ DSParentg, const unsigned* __restrict__ DeformStrucRidpg,
		const unsigned* __restrict__ DCellg)
	{
		const unsigned pp = blockIdx.x * blockDim.x + threadIdx.x;
		if (pp < MapNdeformstruc* MapNdeformstruc) {
			const int p1 = pp / MapNdeformstruc;
			const int p2 = pp % MapNdeformstruc;
			const unsigned idparent = DSParentg[p1];
			const unsigned parentp1 = DeformStrucRidpg[idparent];

			//float3 acep1 = make_float3(0, 0, 0);
			float3 acep1 = DSFlForceg[p1];
			...
			const StDeformStrucIntData body1 = DeformStrucDatag[bodyid1];
			//const float mass1 = body1.rho0 * body1.vol0;
			const float mass1 = body1.mass;

			//int ini1, fin1, ini2, fin2, ini3, fin3;
			//int4 nc = DivDatag.nc;
			//cunsearch::InitCte(DCellg[parentp1], DivDatag.scelldiv, nc, DivDatag.cellzero, ini1, fin1, ini2, fin2, ini3, fin3);

			//for (int c3 = ini3; c3 < fin3; c3 += nc.w) {
			//	for (int c2 = ini2; c2 < fin2; c2 += nc.x) {
			//		unsigned pini, pfin = 0;
			//		cunsearch::ParticleRange(c2, c3, ini1, fin1, DivDatag.beginendcell, pini, pfin);
			//		if (pfin) {
			//			for (int p2 = pini; p2 < pfin; p2++) {
							//const typecode codep2 = Codeg[p2];
							const typecode codep2 = arrays.DSCodeg[p2];
							if (CODE_IsDeformStrucAny(codep2)) {
								...
								if (bodyid1 != bodyid2) {
									...
									const float4 posp2 = arrays.DSPos0g[p2] + arrays.DSDispPhig[p2];
									const float drx = float(posp1.x - posp2.x);
									const float dry = float(posp1.y - posp2.y);
									const float drz = float(posp1.z - posp2.z);
									const float rr2 = drx * drx + dry * dry + drz * drz;
									if (rr2 > ALMOSTZERO) {
										const StDeformStrucIntData body2 = DeformStrucDatag[bodyid2];
										const float sum_radii = 1.4*(0.501f * body1.dp + 0.501f * body2.dp);
										...
										if (rr2 < Dpmsq) {
											...
											const float4 velrhop2 = arrays.DSVelg[p2];
											//const float mass2 = body2.rho0 * body2.vol0;
											const float mass2 = body2.mass;
											...
											if (vt_sq > ALMOSTZERO) {
												...
												const float mu_eff = 0.5f * (body1.kfric + body2.kfric);
			...
			//DSFlForceg[p1] = acep1;
			if (acep1.x != 0) atomicAdd(&DSFlForceg[p1].x, acep1.x);
			if (acep1.y != 0) atomicAdd(&DSFlForceg[p1].y, acep1.y);
			if (acep1.z != 0) atomicAdd(&DSFlForceg[p1].z, acep1.z);
		}
	}
	__global__ void DSFlforceDiff(const int np, const float3* cauchystress, float3* cauchydiff,
		const StDeformStrucIntData* __restrict__ DeformStrucDatag, const StDeformStrucIntArraysg arrays)
	{
		...
		float3 cauchyp1 = cauchystress[p1];
		float3 cauchydiffp1; cauchydiffp1.x = cauchydiffp1.y = cauchydiffp1.z = 0.0f;
		float kersum = 0.0f;
#pragma unroll 8
		for (unsigned pair = 0; pair < pairnsp1.x; ++pair)
		{
			...
			const float3 cauchyp2 = cauchystress[p2];

			cauchydiffp1.x = cauchydiffp1.x - (cauchyp1.x - cauchyp2.x) * kerd;
			cauchydiffp1.y = cauchydiffp1.y - (cauchyp1.y - cauchyp2.y) * kerd;
			cauchydiffp1.z = cauchydiffp1.z - (cauchyp1.z - cauchyp2.z) * kerd;

			kersum = kersum + kerd;
		}
		if (kersum > ALMOSTZERO) {
			cauchydiffp1.x = cauchydiffp1.x / kersum;
			cauchydiffp1.y = cauchydiffp1.y / kersum;
			cauchydiffp1.z = cauchydiffp1.z / kersum;
		}

		cauchydiff[p1] = cauchydiffp1;
	}

	__global__ void DSSmoothFlForce(const int np, float3* cauchystress, const float3* cauchydiff,
		const StDeformStrucIntData* __restrict__ DeformStrucDatag, const StDeformStrucIntArraysg arrays)
	{
		const int p1 = blockIdx.x * blockDim.x + threadIdx.x;
		if (p1 >= np) return;
		float3 cauchyp1 = cauchystress[p1];
		float3 cauchydiffp1 = cauchydiff[p1];
		float3 cauchyp1new;
		const float factor = 0.5f;

		cauchyp1new.x = cauchyp1.x + cauchydiffp1.x * factor;
		cauchyp1new.y = cauchyp1.y + cauchydiffp1.y * factor;
		cauchyp1new.z = cauchyp1.z + cauchydiffp1.z * factor;

		cauchystress[p1] = cauchyp1new;
	}
	void DSInteraction_ForcesDem(bool simulate2d, unsigned MapNdeformstruc, unsigned bodycnt,
		...
		const unsigned* DSParentg, const unsigned* DeformStrucRidpg, const unsigned* Dcellg)
	{
		if (MapNdeformstruc) {
			dim3 sgrid = GetSimpleGridSize(MapNdeformstruc * MapNdeformstruc, SPHBSIZE);
			const size_t shmem = size_t(bodycnt) * sizeof(StDeformStrucIntData);
			DSKerResetForcesDem<< <sgrid, SPHBSIZE >> > (MapNdeformstruc, DSFlForceg);
			if (simulate2d)
				DSKerInteractionForcesDem<true> << <sgrid, SPHBSIZE, shmem >> > (MapNdeformstruc, bodycnt,
					DeformStrucDatag, arrays, DivDatag, Codeg, Posxyg, Poszg, velrhop,
					DSFlForceg, Dp, dtdem, DSSurfPartListg, DSContPowerCoeff, DSParentg, DeformStrucRidpg, Dcellg);
			else
				DSKerInteractionForcesDem<false> << <sgrid, SPHBSIZE, shmem >> > (MapNdeformstruc, bodycnt,
					DeformStrucDatag, arrays, DivDatag, Codeg, Posxyg, Poszg, velrhop,
					DSFlForceg, Dp, dtdem, DSSurfPartListg, DSContPowerCoeff, DSParentg, DeformStrucRidpg, Dcellg);
			

			float3* d_DSFlForceg_diff;
			cudaMalloc(&d_DSFlForceg_diff, MapNdeformstruc * sizeof(float3));
			cudaMemset(d_DSFlForceg_diff, 0, MapNdeformstruc * sizeof(float3));
			DSFlforceDiff << <sgrid, SPHBSIZE >> > (MapNdeformstruc, DSFlForceg, d_DSFlForceg_diff, DeformStrucDatag, arrays);

			DSSmoothFlForce << <sgrid, SPHBSIZE >> > (MapNdeformstruc, DSFlForceg, d_DSFlForceg_diff, DeformStrucDatag, arrays);


			cudaFree(d_DSFlForceg_diff);
		}
	}
...
//	__global__ void KerCountDeformStrucPairs(unsigned n, const float4* poscell0, unsigned* numpairs) {
//		const unsigned p = blockIdx.x * blockDim.x + threadIdx.x; //-Number of thread.
//		if (p < n) {
...
//
//			//-Loop through other deformable structure particles.
...
//
//	//==============================================================================
//	/// Counts the total number of deformable structure pairs (neighbours).
//	/// Cuenta el número total de pares de estructuras deformables (vecinos).
//	//==============================================================================
//	unsigned CountDeformStrucPairs(unsigned npfs, const float4* poscell0, unsigned* numpairs) {
//		if (npfs) {
//			dim3 sgridb = GetSimpleGridSize(npfs, SPHBSIZE);
//			KerCountDeformStrucPairs << <sgridb, SPHBSIZE >> > (npfs, poscell0, numpairs);
...
//
//	//==============================================================================
//	/// Sets the indices for each deformable structure pair.
//	/// Establece los índices para cada par de estructuras deformables.
//	//==============================================================================
//	__global__ void KerSetDeformStrucPairs(unsigned n, const float4* poscell0, unsigned** pairidx)
//	{
//		const unsigned p = blockIdx.x * blockDim.x + threadIdx.x; //-Number of thread.
//		if (p < n) {
//			const unsigned pfs1 = p;      //-Number of particle.
...
//	/// Establece los índices para cada par de estructuras deformables.
//	//==============================================================================
//	void SetDeformStrucPairs(unsigned npfs, const float4* poscell0, unsigned** pairidx) {
//		if (npfs) {
//			dim3 sgridb = GetSimpleGridSize(npfs, SPHBSIZE);
//			KerSetDeformStrucPairs << <sgridb, SPHBSIZE >> > (npfs, poscell0, pairidx);
//		}
//	}
//
//	//==============================================================================
//	/// Calculates the kernel correction matrix for each deformable structure particle.
//	/// Calcula la matriz de corrección del kernel para cada partícula de estructura deformable.
//	//==============================================================================
//	template<TpKernel tker, bool simulate2d> __global__ void KerCalcDeformStrucKerCorr(unsigned n, const typecode* code, const StDeformStrucData* deformstrucdata
//		, const unsigned* deformstrucridp, const float4* poscell0, const unsigned* numpairs, const unsigned* const* pairidx
//		, tmatrix3f* kercorr)
//	{
//		const unsigned p = blockIdx.x * blockDim.x + threadIdx.x; //-Number of thread.
//		if (p < n) {
...
//
//				//-Obtains basic data of particle p1.
//				const float vol0p1 = deformstrucdata[CODE_GetIbodyDeformStruc(code[deformstrucridp[pfs1]])].vol0;
//				const float4 pscell0p1 = poscell0[pfs1];
//
...
//	/// Calcula la matriz de corrección del kernel para cada partícula de estructura deformable.
//	//==============================================================================
//	template<TpKernel tker, bool simulate2d> void CalcDeformStrucKerCorrT(const StInterParmsDeformStrucg& tfs) {
//		if (tfs.vnpfs) {
//			dim3 sgridb = GetSimpleGridSize(tfs.vnpfs, SPHBSIZE);
//			KerCalcDeformStrucKerCorr<tker, simulate2d> << <sgridb, SPHBSIZE >> >
//				(tfs.vnpfs, tfs.code, tfs.deformstrucdata, tfs.deformstrucridp, tfs.poscell0, tfs.numpairs, tfs.pairidx, const_cast<tmatrix3f*>(tfs.kercorr));
//		}
//	}
//
//	//==============================================================================
...
//	/// Calcula la matriz de corrección del kernel para cada partícula de estructura deformable.
//	//==============================================================================
//	template<TpKernel tker> void CalcDeformStrucKerCorr_gt0(const StInterParmsDeformStrucg& tfs) {
//		if (tfs.simulate2d)CalcDeformStrucKerCorrT<tker, true>(tfs);
//		else              CalcDeformStrucKerCorrT<tker, false>(tfs);
//	}
//
//	//==============================================================================
//	/// Calculates the kernel correction matrix for each deformable structure particle.
//	/// Calcula la matriz de corrección del kernel para cada partícula de estructura deformable.
//	//==============================================================================
//	void CalcDeformStrucKerCorr(const StInterParmsDeformStrucg& tfs) {
//#ifdef FAST_COMPILATION
//		if (tfs.tkernel != KERNEL_Wendland)throw "Extra kernels are disabled for FastCompilation...";
//		CalcDeformStrucKerCorr_gt0<KERNEL_Wendland>(tfs);
//#else
//		if (tfs.tkernel == KERNEL_Wendland)     CalcDeformStrucKerCorr_gt0<KERNEL_Wendland>(tfs);
//#ifndef DISABLE_KERNELS_EXTRA
//		else if (tfs.tkernel == KERNEL_Cubic)   CalcDeformStrucKerCorr_gt0<KERNEL_Cubic   >(tfs);
//#endif
//#endif
//	}
//
//	//==============================================================================
//	/// Calculates the deformation gradient matrix for each deformable structure particle.
//	/// Calcula la matriz de gradiente de deformación para cada partícula de estructura deformable.
//	//==============================================================================
//	template<TpKernel tker, bool simulate2d> __global__ void KerComputeDefGradDeformStruc(unsigned n, const float4* poscell, const typecode* code
//		, const StDeformStrucData* deformstrucdata, const unsigned* deformstrucridp
...
//
//				//-Obtains basic data of particle p1.
//				const unsigned p1 = deformstrucridp[pfs1];
//				const float vol0p1 = deformstrucdata[CODE_GetIbodyDeformStruc(code[p1])].vol0;
//				const float4 pscellp1 = poscell[p1];
//				const float4 pscell0p1 = poscell0[pfs1];
...
//				for (unsigned pair = 0; pair < numpairsp1; pair++) {
//					const unsigned pfs2 = pairidx[pfs1][pair];
//					const unsigned p2 = deformstrucridp[pfs2];
//					const float4 pscellp2 = poscell[p2];
//					const float4 pscell0p2 = poscell0[pfs2];
//					const float drx = pscellp1.x - pscellp2.x + CTE.poscellsize * (PSCEL_GetfX(pscellp1.w) - PSCEL_GetfX(pscellp2.w));
//					const float dry = pscellp1.y - pscellp2.y + CTE.poscellsize * (PSCEL_GetfY(pscellp1.w) - PSCEL_GetfY(pscellp2.w));
...
//
//	//==============================================================================
//	/// Calculates the PK1 stress matrix for each deformable structure particle.
//	/// Calcula la matriz de tensión PK1 para cada partícula de estructura deformable.
//	//==============================================================================
//	__device__ tmatrix3f KerComputePK1StressDeformStruc(const tmatrix3f& defgrad, const tmatrix6f& cmat)
//	{
...
//
//	//==============================================================================
//	/// Interaction forces for the deformable structure particles.
//	/// Fuerzas de interacción para las partículas de estructura deformable.
//	//==============================================================================
//	template<TpKernel tker, bool simulate2d, bool lamsps> __global__ void KerInteractionForcesDeformStruc(unsigned n, float visco
//		, int scelldiv, int4 nc, int3 cellzero, const int2* beginendcellfluid, const unsigned* dcell
...
//		, float* deformstrucdt, float3* ace)
//	{
//		//const unsigned p = blockIdx.x * blockDim.x + threadIdx.x; //-Number of thread.
//		//if (p < n) {
//		//	const unsigned pfs1 = p;      //-Number of particle.
...
//		//	if (CODE_IsDeformStrucDeform(codep1)) {
//		//		float3 acep1 = make_float3(0, 0, 0);
//
//		//		//-Obtains basic data of particle p1.
//		//		const float4 pscellp1 = poscell[p1];
...
//
//		//		//-Obtains deformable structure data.
//		//		const float vol0p1 = deformstrucdata[CODE_GetIbodyDeformStruc(codep1)].vol0;
//		//		const float rho0p1 = deformstrucdata[CODE_GetIbodyDeformStruc(codep1)].rho0;
//		//		const float youngmod = deformstrucdata[CODE_GetIbodyDeformStruc(codep1)].youngmod;
//		//		const float poissratio = deformstrucdata[CODE_GetIbodyDeformStruc(codep1)].poissratio;
//		//		const float hgfactor = deformstrucdata[CODE_GetIbodyDeformStruc(codep1)].hgfactor;
//		//		const tmatrix6f cmat = deformstrucdata[CODE_GetIbodyDeformStruc(codep1)].cmat;
//		//		const tmatrix3f pk1p1 = KerComputePK1StressDeformStruc(defgradp1, cmat);
//		//		const tmatrix3f pk1kercorrp1 = cumath::MulMatrix3x3(pk1p1, kercorrp1);
//
//		//		//-Get current mass of deformable structure particle.
...
//		//		cunsearch::InitCte(dcell[p1], scelldiv, nc, cellzero, ini1, fin1, ini2, fin2, ini3, fin3);
//
//		//		//-Deformable structure-Fluid interaction.
//		//		for (int c3 = ini3; c3 < fin3; c3 += nc.w) {
//		//			for (int c2 = ini2; c2 < fin2; c2 += nc.x) {
//		//				unsigned pini, pfin = 0;  cunsearch::ParticleRange(c2, c3, ini1, fin1, beginendcellfluid, pini, pfin);
//		//				if (pfin) {
...
//		//			const tmatrix3f pk1p2 = KerComputePK1StressDeformStruc(defgradp2, cmat);
...
//		//			float3 r = ace[p1]; r.x += acep1.x; r.y += acep1.y; r.z += acep1.z; ace[p1] = r;
//		//			if (csp1 > deformstrucdt[pfs1])deformstrucdt[pfs1] = csp1;
//		//		}
//		//	}
//		//}
//	}
...
//	/// Fuerzas de interacción para las partículas de estructura deformable.
//	//==============================================================================
//	template<TpKernel tker, bool simulate2d, bool lamsps> void Interaction_ForcesDeformStrucT(const StInterParmsDeformStrucg& tfs) {
//		if (tfs.vnpfs) {
//			const StDivDataGpu& dvd = tfs.divdatag;
//			dim3 sgridb = GetSimpleGridSize(tfs.vnpfs, SPHBSIZE);
//			KerComputeDefGradDeformStruc<tker, simulate2d> << <sgridb, SPHBSIZE, 0, tfs.stm >> >
//				(tfs.vnpfs, tfs.poscell, tfs.code, tfs.deformstrucdata, tfs.deformstrucridp, tfs.poscell0, tfs.numpairs, tfs.pairidx, tfs.kercorr, tfs.defgrad);
//			KerInteractionForcesDeformStruc<tker, simulate2d, lamsps> << <sgridb, SPHBSIZE, 0, tfs.stm >> >
//				(tfs.vnpfs, tfs.viscob, dvd.scelldiv, dvd.nc, dvd.cellzero, dvd.beginendcell + dvd.cellfluid, tfs.dcell, tfs.poscell, tfs.velrhop, tfs.code, tfs.deformstrucdata, tfs.deformstrucridp, tfs.poscell0, tfs.numpairs, tfs.pairidx, tfs.kercorr, tfs.defgrad, tfs.deformstrucdt, tfs.ace);
//		}
//	}
//
//	//==============================================================================
...
//	/// Fuerzas de interacción para las partículas de estructura deformable.
//	//==============================================================================
//	template<TpKernel tker, bool simulate2d> void Interaction_ForcesDeformStruc_gt1(const StInterParmsDeformStrucg& tfs) {
//		if (tfs.lamsps)Interaction_ForcesDeformStrucT<tker, simulate2d, true>(tfs);
//		else          Interaction_ForcesDeformStrucT<tker, simulate2d, false>(tfs);
//	}
//
//	//==============================================================================
//	/// Interaction forces for the deformable structure particles.
//	/// Fuerzas de interacción para las partículas de estructura deformable.
//	//==============================================================================
//	template<TpKernel tker> void Interaction_ForcesDeformStruc_gt0(const StInterParmsDeformStrucg& tfs) {
//		if (tfs.simulate2d)Interaction_ForcesDeformStruc_gt1<tker, true>(tfs);
//		else              Interaction_ForcesDeformStruc_gt1<tker, false>(tfs);
//	}
//
//	//==============================================================================
//	/// Interaction forces for the deformable structure particles.
//	/// Fuerzas de interacción para las partículas de estructura deformable.
//	//==============================================================================
//	void Interaction_ForcesDeformStruc(const StInterParmsDeformStrucg& tfs) {
//#ifdef FAST_COMPILATION
//		if (tfs.tkernel != KERNEL_Wendland)throw "Extra kernels are disabled for FastCompilation...";
//		Interaction_ForcesDeformStruc_gt0<KERNEL_Wendland>(tfs);
//#else
//		if (tfs.tkernel == KERNEL_Wendland)  Interaction_ForcesDeformStruc_gt0<KERNEL_Wendland>(tfs);
//#ifndef DISABLE_KERNELS_EXTRA
//		else if (tfs.tkernel == KERNEL_Cubic)Interaction_ForcesDeformStruc_gt0<KERNEL_Cubic>(tfs);
//#endif
//#endif
//	}
//
...
//	/// Actualizacion de posicion de particulas segun desplazamiento.
//	//==============================================================================
//	__global__ void KerComputeStepPosDeformStruc(unsigned n, const unsigned* deformstrucridp
//		, const double2* posxypre, const double* poszpre, const double2* movxy, const double* movz
//		, double2* posxy, double* posz, unsigned* dcell, typecode* code)
//	{
//		unsigned pt = blockIdx.x * blockDim.x + threadIdx.x; //-Number of particle.
//		if (pt < n) {
//			unsigned p = deformstrucridp[pt];
//			const typecode rcode = code[p];
//			const bool outrhop = CODE_IsOutRhop(rcode);
//			const bool deformstruc = CODE_IsDeformStrucDeform(rcode);
//			const bool normal = (outrhop || CODE_IsNormal(rcode));
//			if (normal && deformstruc) {
//				const double2 rmovxy = movxy[p];
...
//	/// Actualizacion de posicion de particulas segun desplazamiento.
//	//==============================================================================
//	void ComputeStepPosDeformStruc(unsigned npfs, const unsigned* deformstrucridp
//		, const double2* posxypre, const double* poszpre, const double2* movxy, const double* movz
//		, double2* posxy, double* posz, unsigned* dcell, typecode* code)
//	{
//		if (npfs) {
//			dim3 sgrid = GetSimpleGridSize(npfs, SPHBSIZE);
//			KerComputeStepPosDeformStruc << <sgrid, SPHBSIZE >> > (npfs, deformstrucridp, posxypre, poszpre, movxy, movz, posxy, posz, dcell, code);
//		}
//	}
	//<vs_deformstruc_end>
}
