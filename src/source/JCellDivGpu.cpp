	...
	, DirOut(dirout), AllocFullNct(allocfullnct), OverMemoryNp(overmemorynp), OverMemoryCells(overmemorycells)
{
	...
	BeginEndCell = NULL;
	SortPart2 = NULL; SortIdx = NULL; //<vs_deformstruc>
	Reset();
}
...
void JCellDivGpu::FreeMemoryAll() {
	...
	cudaFree(SortPart2);  SortPart2 = NULL; //<vs_deformstruc>
	cudaFree(SortIdx);    SortIdx = NULL;   //<vs_deformstruc>
	MemAllocGpuNp = 0;
	BoundDivideOk = false;
}
...
void JCellDivGpu::AllocMemoryNp(ullong np) {
	...
	cudaMalloc((void**)&SortPart2, m); MemAllocGpuNp += m;
	cudaMalloc((void**)&SortIdx, m); MemAllocGpuNp += m;
	SizeAuxMem = cudiv::LimitsPosSize(SizeNp);
	m = sizeof(float) * SizeAuxMem;
	cudaMalloc((void**)&AuxMem, m);   MemAllocGpuNp += m;
	cudaError_t cuerr = cudaGetLastError();
	if (cuerr != cudaSuccess) {
		Run_ExceptioonCuda(cuerr, fun::PrintStr("Failed GPU memory allocation of %.1f MB for %u particles.", double(MemAllocGpuNp) / (1024 * 1024), SizeNp));
	}
...
void JCellDivGpu::DSAllocMemoryNp(ullong np) {
	FreeMemoryAll();
	SizeNp = unsigned(np);
	if (np != SizeNp)Run_Exceptioon(string("Failed memory allocation for ") + fun::UlongStr(np) + " deformable structure particles.");
	MemAllocGpuNp = 0;
	size_t m = sizeof(unsigned) * SizeNp;
	cudaMalloc((void**)&CellPart, m); MemAllocGpuNp += m;
	cudaMalloc((void**)&SortPart, m); MemAllocGpuNp += m;
	...
	cudaError_t cuerr = cudaGetLastError();
	if (cuerr != cudaSuccess) {
		Run_ExceptioonCuda(cuerr, fun::PrintStr("Failed GPU memory allocation of %.1f MB for %u deformable structure particles.", double(MemAllocGpuNp) / (1024 * 1024), SizeNp));
	}

	Log->Printf("  CellDiv: Requested gpu memory for %u deformable structure particles: %.1f MB.", SizeNp, double(MemAllocGpuNp) / (1024 * 1024));
}
...
void JCellDivGpu::DSCheckMemoryNp(unsigned npmin) {
	DSAllocMemoryNp(npmin);
	IncreaseNp = 0;
}
...
void JCellDivGpu::CheckMemoryNct(unsigned nctmin) {
	...
	else if (!BeginEndCell) {
		AllocMemoryNct(SizeNct);
	}
}

void JCellDivGpu::DSCheckMemoryNct(unsigned nctmin) {
	FreeMemoryNct();
	SizeNct = unsigned(nctmin);

	if (nctmin != SizeNct)Run_Exceptioon(string("Failed GPU memory allocation for ") + fun::UlongStr(nctmin) + " cells.");
	...
	cudaError_t cuerr = cudaGetLastError();
	if (cuerr != cudaSuccess) {
		Run_ExceptioonCuda(cuerr, fun::PrintStr("Failed GPU memory allocation of %.1f MB for %u deformable structure cells.", double(MemAllocGpuNct) / (1024 * 1024), SizeNct));
	}

	Log->Printf("  CellDiv: Requested gpu memory for %u deformbale structure cells (CellMode=%s): %.1f MB.", SizeNct, GetNameCellMode(CellMode), double(MemAllocGpuNct) / (1024 * 1024));
}
...
//==============================================================================
void JCellDivGpu::SortDataArrays(const unsigned* a1, const typecode* b1, const unsigned* c1, const float4* d1,
	unsigned* a2, typecode* b2, unsigned* c2, float4* d2) 
{
	cudiv::SortDataParticles(Nptot, 0, SortPart, a1, b1, c1, d1, a2, b2, c2, d2);
}
...
tdouble3 JCellDivGpu::GetDomainLimits(bool limitmin, unsigned slicecellmin)const {
	...
	const double scell = double(Scell);
...
//<vs_deformstruc_ini>
//==============================================================================
/// Sorts and updates the indices of the deformable structure particles.
/// Ordena y actualiza los índices de las partículas de estructura deformable.
//==============================================================================
