...
void JCellDivGpuSingle::DSCalcCellDomain() {
	CellDomainMin = TUint3(0, 0, 0);
	CellDomainMax = TUint3(DomCells.x - 1, DomCells.y - 1, DomCells.z - 1);
	if (!BoundLimitOk) {
		BoundLimitOk = true; BoundLimitCellMin = CellDomainMin; BoundLimitCellMax = CellDomainMax;
...
void JCellDivGpuSingle::DSPrepareNct() {
...
void JCellDivGpuSingle::DSPreSort(const unsigned* dcellg, const typecode* codeg) {
	cudiv::PreSortFull(Nptot, DomCellCode, dcellg, codeg, CellDomainMin, TUint3(Ncx, Ncy, Ncz), CellPart, SortPart);
}
...
void JCellDivGpuSingle::DSContactDivide(unsigned npb1, unsigned npf1, unsigned npb2, unsigned npf2
	, bool boundchanged, const unsigned* dcellg, const typecode* codeg
	, const double2* posxy, const double* posz, const unsigned* idp, JDsTimersGpu* timersg)
{
	DivideFull = false;
