...
class JSphInOut;
class JSphDeformStruc;  //<vs_deformstruc>
class JDsPartsInit;
class JDsPips;
class JLinearValue;
class JCaseEParms;
...
protected:
	...
	JSphDeformStruc* DeformStruc; ///<Object for deformable structures.

	JUserExpressionList* UserExpressions; ///<Object for user mathematical expressions.
	...
	float DSPosCellSize;       ///<Size of cells used for coding PosCell on GPU (it is usually KernelSize).
	...
	std::string DSOutFile;						///<Output file for time dependent variables
	std::vector<std::string> DSMeasPlOutFiles;	///<Output file for time dependent variables for measuring planes
	//double DeformStrucCs0;						///<Maximum initial speed of sound across all deformable structures.
	unsigned DeformStrucCount;					///<Number of deformable structure bodies
	unsigned CaseNdeformstruc;					///<Number of original deformable structure particles (befor mapping)
	int MapNdeformstruc;						///<Number of mapped deformable structure particles (after mapping)
	unsigned DSNumPairs;						///<Total number of pairs across all deformable structure bodies.
	bool DSFracture;							///<Switch if any of the bodies require fracture solving
	bool DSPlastic;								///<Switch if any of the bodies require plasticity solving
	int DSNpSurf;								///<Number of particles on the surface of deformable structures
	int DSNstep;								///<Number of step in execution for deformable structure.
	int DSPartNstep;							///<Number of step when last PART was saved for deformable structure.
	double DSMaxDt;								///<Maximum allowable dt across all the deformable bodies
	double DSSymplecticDtPre;
	unsigned DSNMeasPlanes;						///<Number of measuring planes
	unsigned DSNPartMeasPlanes;					///<Total number of particles on the measuring planes
	float DSContPowerCoeff;						///<Coefficient to multiply contact power with ge 1.0
	tuint3 DSDomCelIni;				///<First cell within the Map defining local simulation area. DomCelIni=TUint3(0) for Single-CPU
	tuint3 DSDomCelFin;				///<Last cell within the Map defining local simulation area. DomCelFin=Map_Cells for Single-CPU
	tuint3 DSDomCells;				///<Number of cells in each direction. DomCells=DomCelFin-DomCelIni                                                             
	tdouble3 DSDomPosMin;			///<Lower limit of simulation + edge (KernelSize) if periodic conditions. DomPosMin=Map_PosMin+(DomCelIni*Scell); 
	tdouble3 DSDomPosMax;			///<Upper limit of simulation + edge (KernelSize) if periodic conditions. DomPosMax=min(Map_PosMax,Map_PosMin+(DomCelFin*Scell));
	tdouble3 DSDomSize;				///<Result of DomSize = DomPosMax - DomPosMin
	tdouble3 DSDomRealPosMin;		///<Real lower limit of the simulation according to DomCelIni/Fin, DomRealPosMin=max(DomPosMin,MapRealPosMin)
	tdouble3 DSDomRealPosMax;		///<Real upper limit of the simulation according to DomCelIni/Fin, DomRealPosMax=min(DomPosMax,MapRealPosMax)
	unsigned DSDomCellCode;			///<Key for encoding cell position within the Domain.
	tdouble3 DSMap_PosMin;			///<Lower limit of simulation + edge (KernelSize) if periodic conditions. Map_PosMin=MapRealPosMin-KernelSize(in periodic axis)
	tdouble3 DSMap_PosMax;			///<Upper limit of simulation + edge (KernelSize) if periodic conditions. Map_PosMax=MapRealPosMax+KernelSize(in periodic axis)
	tdouble3 DSMap_Size;			///<Result of Map_Size = Map_PosMax - Map_PosMin
	tuint3 DSMap_Cells;				///<Maximum number of cells within case limits. Map_Cells=TUint3(unsigned(ceil(Map_Size.xyz/Scell))
	float DScellsize;				///<Cell size: maximum kernelsize.
	unsigned DSDtModif;				///<Number of modifications on  dt computed when it is too low.  
	unsigned DSDtModifWrn;			///<Limit number for warning generation.

	tdouble3 DSCalcEnergies(const tmatrix3d defgrad, const tdouble3 velp1,
		...
		const double eqplastic, const tmatrix3d plasticdev, const StDeformStrucIntData bod)const;
	void DSGatherArray(const unsigned npfs, const unsigned* dsiridp, const tdouble2* fullarray, tfloat2* dsarray) const;
	void DSGatherArray(const unsigned npfs, const unsigned* dsiridp, const double* fullarray, float* dsarray) const;
	void DSSaveMapCellsVtk()const;
protected:
	...
	void DSLoadDcellParticles(int n, const typecode* code, const tdouble3* pos0, const tdouble3* disp, unsigned* dcell)const;
	void DSLoadDcellParticles(int n, const typecode* code, const tfloat4* pos0, const tfloat4* disp, unsigned* dcell)const;
	void DSLoadDcellSurfParticles(int n, const unsigned* DSSurfPartList, const typecode* code, const tdouble3* pos0, const tdouble3* disp, unsigned* dcell)const;
	...
	void DSSelecDomain(tuint3 celini, tuint3 celfin);
	void ConfigPosCellGpu();
	void DSConfigPosCellGpu(float kernelsize);
	...
	void PrintHeadPart(bool defstruc);
	...
	void AddBasicArraysDS(JDataArrays& arrays, unsigned np, const tdouble3* pos, const tdouble3* disp,
		...
		const unsigned DSNMeasPlanes, const unsigned* DSMeasPlnCnt, const unsigned DSNPartMeasPlanes, const unsigned* DSMeasPlnPart, const float* DSKerSumVol)const;
	void AddBasicArraysDS(JDataArrays& arrays, unsigned np, const tfloat4* pos,
		...
		const unsigned* DSMeasPlnPart, const float* DSKerSumVolc, const tfloat3* DSFluidForce)const;
	void EnsureDsDerivedArrays(JDataArrays& arrays, StDeformStrucIntData* deformstrucdata, unsigned dsnp)const;

	void SavePartData(unsigned npok, unsigned nout, const JDataArrays& arrays, const JDataArrays& dsarrays, unsigned ndom, const tdouble3* vdom, const StInfoPartPlus* infoplus, 
		StDeformStrucIntData* deformstrucdata, bool floatarrays, const unsigned DSNMeasPlanes, const unsigned DSNPartMeasPlanes);
	void SaveData(unsigned npok, const JDataArrays& arrays, const JDataArrays& DSarrays, unsigned ndom, 
		const tdouble3* vdom, const StInfoPartPlus* infoplus, StDeformStrucIntData* deformstrucdata, bool floatarrays, const unsigned DSNMeasPlanes, const unsigned DSNPartMeasPlanes);
