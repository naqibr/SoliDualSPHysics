...
namespace fsph {
	...
	//============================================================================== 
	/// Returns kernel of TLSPH kernel according to template for variable kernelh of domains.
	//==============================================================================
	template<TpKernel tker> inline float GetTLSPHKernel_Wab(const bool simulate2d, const float kernelh, const float rr2) {
		if (tker == KERNEL_Wendland) {
			StKWendlandCte csp = GetKernelWendland_Ctes(simulate2d, kernelh);
			return(GetKernelWendland_Wab(csp, kernelh, rr2));
		}
		else if (tker == KERNEL_Cubic) {
			StKCubicCte csp = GetKernelCubic_Ctes(simulate2d, kernelh);
			return(GetKernelCubic_Wab(csp, kernelh, rr2));
		}
		else return(0);
	}
	...
	//==============================================================================
	inline float GetTLSPHKernel_Wab(TpKernel tker, const bool simulate2d, const float kernelh, const float rr2) {
		if (tker == KERNEL_Wendland) {
			StKWendlandCte csp = GetKernelWendland_Ctes(simulate2d, kernelh);
			return(GetKernelWendland_Wab(csp, kernelh, rr2));
		}
	...
	//==============================================================================
	template<TpKernel tker> inline float GetTLSPHKernel_WabFac(const bool simulate2d, const float kernelh, const float rr2, float& fac) {
		if (tker == KERNEL_Wendland) {
			StKWendlandCte csp = GetKernelWendland_Ctes(simulate2d, kernelh);
			return(GetKernelWendland_WabFac(csp, kernelh, rr2, fac));
		}
		else if (tker == KERNEL_Cubic) {
			StKCubicCte csp = GetKernelCubic_Ctes(simulate2d, kernelh);
			return(GetKernelCubic_WabFac(csp, kernelh, rr2, fac));
		}
		else return(0);
	}
