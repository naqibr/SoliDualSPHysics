...
unsigned JXml::GetAttributeUnsigned(const TiXmlElement* ele
	, const std::string& attr, bool optional, unsigned valdef)const
{
	int ret = GetAttributeInt(ele, attr, optional, int(valdef));
	double retdbl = GetAttributeDouble(ele, attr, optional, double(valdef));
	if (retdbl < 0)ErrReadAtrib(ele, attr, false);
	return(unsigned(ret));
}
...
//==============================================================================
unsigned JXml::GetAttributeUnsignedn(const TiXmlElement* ele, const std::string& name
	, const std::string& attr, bool optional, unsigned valdef)const
{
	int ret = GetAttributeInt(ele, attr, optional, int(valdef));
	double retdbl = GetAttributeDouble(ele, attr, optional, double(valdef));
	if (retdbl < 0)ErrReadAtrib(ele, name, false, "Cannot be negative.");
	return(unsigned(ret));
}
...
/// \throw JException The requested attribute does not exist...
//==============================================================================
double JXml::GetAttributeDoublePos(const TiXmlElement* ele, const std::string& name, const std::string& attr
	, bool optional, double valdef)const
{
	double ret;
	const char* vchar = ele->Attribute(attr.c_str());
	if (vchar == NULL) {
		if (optional)ret = valdef;
		else ErrReadAtrib(ele, attr, true);
	}
	else {
		if (NuxLib != NULL && vchar[0] == '#') {
#ifndef DISABLE_NUMEXLIB
			...
			if (!fun::StrIsRealNumber(vchar))ErrReadAtrib(ele, attr, false, "It is not a valid real number.");
			ret = atof(vchar);
			if (ret < 0.0) ErrReadAtrib(ele, name, false, "Cannot be negative.");
		}
	}
	return(ret);
}
...
//==============================================================================
bool JXml::CheckAttribute(const TiXmlElement* ele, const std::string& name)const
{
	const char* vchar = ele->Attribute(name.c_str());
...
//==============================================================================
unsigned JXml::ReadArrayBCstruc(const TiXmlNode* node, const std::string& name
	, tbcstrucbody* vec, unsigned count) const
{
	if (count) {
		TiXmlElement* ele = GetFirstElement(node, name, true);
		for (unsigned c = 0; c < count && ele; c++) {
			vec[c] = GetAttributeBCstruc(ele);
			ele = GetNextElement(ele, name, true);
		}
	}
	return(count);
...
/// \read NULL if Values missing or any value is not valid...
//==============================================================================
unsigned JXml::ReadArrayNotchstruc(const TiXmlNode* node, const std::string& name
	, plane4Nstruc* vec, unsigned count) const
{
	if (count) {
		TiXmlElement* ele = GetFirstElement(node, name, true);
		for (unsigned c = 0; c < count && ele; c++) {
			vec[c] = GetAttributeNotchstruc(ele);
			ele = GetNextElement(ele, name, true);
		}
	}
	return(count);
