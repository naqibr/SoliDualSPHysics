...
typedef struct StDeformStrucIntArraysg {
	...
	matrix3f* DSPlasticFpg;				///<Plastic deformation gradient tensor per particle [MapNdeformstruc].
	...
	StDeformStrucIntArraysg(typecode* _DSCodeg, unsigned* _DSPairJg, uint2* _DSPairNSg,
		...
		float4* _DSPos0g, tbcstruc* _DSPartFBCg, tphibc* _DSPartPhiBCg, float4* _DSDispPhig, float* _DSEqPlasticg, tmatrix3f* _DSPlasticStraing,matrix3f* _DSPlasticFpg, float4* _DSDefGradg2D,
		...
		DSDispCorxzg(_DSDispCorxzg), DSDispCoryg(_DSDispCoryg), DeformStruc(_DeformStruc), DSEqPlasticg(_DSEqPlasticg), DSPlasticStraing(_DSPlasticStraing), DSPlasticFpg(_DSPlasticFpg)
	{
	}
    ...
    StDeformStrucIntArraysg() :
		...
		DeformStruc(nullptr), DSEqPlasticg(nullptr), DSPlasticStraing(nullptr), DSPlasticFpg(nullptr)
    {
    }
