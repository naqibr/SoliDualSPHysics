...
void JCellDivCpu::FreeMemoryNp() {
	...
	delete[] SortIdx;     SortIdx = NULL;   //<vs_deformstruc>
	MemAllocNp = 0;
	BoundDivideOk = false;
}
...
void JCellDivCpu::AllocMemoryNp(ullong np) {
	...
	try {
		...
		SortIdx = new unsigned[SizeNp];                       MemAllocNp += sizeof(unsigned) * SizeNp;  //<vs_deformstruc>
	}
...
void JCellDivCpu::DSAllocMemoryNp(ullong np) {
	FreeMemoryNp();
	SizeNp = unsigned(np);
	//-Check number of particles | Comprueba numero de particulas.
	if (np != SizeNp)Run_Exceptioon(string("Failed memory allocation for ") + fun::UlongStr(np) + " particles.");
...
void JCellDivCpu::DSCheckMemoryNp(unsigned npmin) {
...
void JCellDivCpu::DSCheckMemoryNct(unsigned nctmin) {
	AllocMemoryNct(nctmin);
}
...
//==============================================================================
/// Sorts the indices of the deformable structure particles.
/// Ordena los índices de las partículas de estructura deformable.
//==============================================================================
...
//==============================================================================
/// Updates the indices of the deformable structure particles.
/// Actualiza los índices de las partículas de estructura deformable..
//==============================================================================
