/*
 ...
 SoliDualSPHysics extends DualSPHysics to solid mechanics with hyperelasticity, 
 plasticity, and fracture capabilities.
 ...
 These extensions are distributed under the same GNU LGPL license as DualSPHysics.
*/
...
JAppInfo AppInfo("DualSPHysics5","v5.2.269","DeformStruc","v1.0","03-04-2023");
...
std::string getlicense_lgpl(const std::string &name,bool simple){
  ...
  if(!simple){
    ...
    tx=tx+"\n Properly cite the use of DualSPHysics (Dominguez et al., 2022) (https://dual.sphysics.org/references).\n";
    tx=tx+"\n ---\n";
    tx=tx+"\n <SOLIDUALSPHYSICS> SoliDualSPHysics Extensions Copyright (c) 2024 by";
    ...
    tx=tx+"\n Developed as part of the PhD thesis by Dr. Naqib Rahimi:";
    tx=tx+"\n \"Computational Mechanics of Extreme Events: Advanced Multi-physics Modeling";
    tx=tx+"\n and Simulations with Smoothed Particle Hydrodynamics, Isogeometric Analysis,";
    tx=tx+"\n and Phase Field\", supervised by Dr. Georgios Moutsanidis.\n";
    tx=tx+"\n Related publication:";
    tx=tx+"\n Rahimi, N., & Moutsanidis, G. (2024). \"SoliDualSPHysics: An extension of";
    tx=tx+"\n DualSPHysics to solid mechanics with hyperelasticity, plasticity, and fracture.\"";
    tx=tx+"\n [In preparation]\n";
    tx=tx+"\n SoliDualSPHysics extensions are distributed under the same GNU LGPL license.\n\n";
  }
  return(tx);
}
...
bool ShowsVersionInfo(int argc,char** argv){
  ...
  if(fun::StrRemoveAfter(option,":")=="-ver"){
    ...
    features.push_back(fun::JSONProperty("DeformStruc",true)); //<vs_deformstruc>
...
int main(int argc, char** argv){
  ...
  //AppInfo.AddNameExtra("DeformStruc");   //<vs_deformstruc>
  #ifdef CODE_SIZE4
