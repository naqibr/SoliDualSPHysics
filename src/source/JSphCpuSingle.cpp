...
void JSphCpuSingle::DSComputeStep_Sym(const double dt, double starttime) {
	...
	while (subdt < dt) {
		...
		double dsdt = defsttm ? DeformStruc->UseUsrTimeStep : CFLnumber * DSMaxDt;
		//if (!defsttm && dsdt < double(DtMin)) {
		//	dsdt = double(DtMin); DSDtModif++;
		//	if (DSDtModif >= DSDtModifWrn) {
		//		Log->PrintfWarning("%d DTs adjusted to DtMin for deformable structure (t:%g, nstep:%u)", DSDtModif, TimeStep, DSNstep);
		//		DSDtModifWrn *= 10;
		//	}
...
#pragma omp for
		...
		for (int p1 = 0; p1 < MapNdeformstruc; p1++)
		{
			...
			if (constmdl == CONSTITMODEL_SVK) {
				...
				tmatrix3d cauchy_green = fmath::MulMatrix3x3(defgradT, defgradp1);
				const double jac = simulate2d ? fmath::Determinant2x2(defgradp1) : fmath::Determinant3x3(defgradp1);

				// Load plastic metric tensor C_p (stored in Lagrangian config)
				tmatrix3d Cp = DSPlasticStrain[p1];

				// Compute elastic right Cauchy-Green: C_e = C * C_p^{-1}
				tmatrix3d Cp_inv;
				if (simulate2d) {
					Cp_inv = fmath::InverseMatrix2x2(Cp);
					Cp_inv.a22 = 1.0;
				}
				else {
					Cp_inv = fmath::InverseMatrix3x3(Cp);
				}
				tmatrix3d C_e = fmath::MulMatrix3x3(cauchy_green, Cp_inv);

				// Elastic Jacobian: J_e = sqrt(det(C_e)) = J / J_p
				const double det_Ce = simulate2d ? fmath::Determinant2x2(C_e) : fmath::Determinant3x3(C_e);
				...
				const double mean_Ce_bar = tr_Ce_bar / 3.0;
				tmatrix3d dev_Ce_bar = C_e_bar;
				...
				const double M_trial_sq =
					M_dev_trial.a11 * M_dev_trial.a11 +
					M_dev_trial.a22 * M_dev_trial.a22 +
					M_dev_trial.a33 * M_dev_trial.a33 +
					2.0 * (M_dev_trial.a12 * M_dev_trial.a12 +
						M_dev_trial.a13 * M_dev_trial.a13 +
						M_dev_trial.a23 * M_dev_trial.a23);
				const double sigma_eq_trial = sqrt(max(1.5 * M_trial_sq, 0.0));

				double plastic_strain = (DSEqPlastic ? DSEqPlastic[p1] : 0.0);
				const double sigma_yield = max(double(body.yieldstress) + body_hard * plastic_strain, 0.0);
				tmatrix3d M_dev = M_dev_trial;
				tmatrix3d Cp_new = Cp;
				if (sigma_eq_trial > sigma_yield + double(ALMOSTZERO)) {
					const float denom = 3.0f * body_mu + body_hard * 0.816496580927726f;
					double dgamma = (sigma_eq_trial - sigma_yield) / denom;
					dgamma = max(dgamma, 0.0);
					const double ratio = max(1.0 - (3.0 * body_mu * dgamma) / sigma_eq_trial, 0.0);
					...
					const double norm_M = max(sqrt(M_trial_sq), double(ALMOSTZERO));
					const double factor = 3.0 * dgamma / norm_M;

					tmatrix3d flow_Cp = fmath::MulMatrix3x3(M_dev_trial, Cp);
					...
					// Normalize Cp_new to ensure det(Cp_new)^(1/3) = 1 (plastic incompressibility)
					double detCpNew = simulate2d ? fmath::Determinant2x2(Cp_new) : fmath::Determinant3x3(Cp_new);
					detCpNew = max(detCpNew, 1e-12);
					double s = pow(detCpNew, -1.0 / 3.0);
				...
				tmatrix3d C_inv;
				if (simulate2d) {
					C_inv = fmath::InverseMatrix2x2(cauchy_green);
					C_inv.a22 = 1.0;
				}
				else {
					C_inv = fmath::InverseMatrix3x3(cauchy_green);
				}
				const double bulk = body_lambda + (2.0 / 3.0) * body_mu;
				const double vol_coeff = 0.5 * bulk * (jac * jac - 1.0);

				// Deviatoric part: S_dev = J_e^{-1} * C_e^{-1} * M_dev * C_e^{-1}
				// Note: C_e^{-1} = (C * C_p^{-1})^{-1} = C_p * C^{-1}
				// Use updated Cp_new after plastic flow
				tmatrix3d Cp_Cinv = fmath::MulMatrix3x3(Cp_new, C_inv);
				tmatrix3d Ce_inv_M = fmath::MulMatrix3x3(Cp_Cinv, M_dev);
				tmatrix3d S_dev = fmath::MulMatrix3x3(Ce_inv_M, Cp_Cinv);
				const double Je_inv = 1.0 / max(Je, 1e-10);
				S_dev = S_dev * Je_inv;
				...
				// Store updated plastic metric and equivalent plastic strain
				if (DSPlasticStrain) DSPlasticStrain[p1] = Cp_new;

				if (DSEqPlastic) DSEqPlastic[p1] = max(plastic_strain, 0.0);
			}
...
#pragma omp for
		...
		for (int p1 = 0; p1 < MapNdeformstruc; p1++)
		{
			...
			if (body.fracture) {
				double aphi = DSPhiDDot[p1];
				double dphi = DSPhiDot[p1];
				aphi = aphi > double(ALMOSTZERO) ? sqrt(body.kernelh / aphi) : DBL_MAX;
				dphi = dphi > double(ALMOSTZERO) ? body.kernelh / dphi : DBL_MAX;
				dt_accl = min(dt_accl, aphi);
...
void JSphCpuSingle::DSPreTimeInt()
{
	...
	{
		DSInteraction_Forces<true, false>(subdt);
		DSCompSemImplEuler(dsdt, subdt);
		subdt += dsdt;
		DSNstep++;
	}
	...
	std::cout.flush();
	JDataArrays dsarrays;
	AddBasicArraysDS(dsarrays, MapNdeformstruc, DSPos0, DSDisp, DSPhi, DSParent, DSCode, DSiBodyRidp,
		DSDefGrad, DSPiolKir, DSVel, DSPairN, DSPairStart, DSPairJ, DSNumPairs, DSKerDerV0);
	DSSaveData(dsarrays, DefStrucIntData, trackparticleid);
	exit(0);*/
}
