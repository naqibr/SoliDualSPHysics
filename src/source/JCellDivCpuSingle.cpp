...
void JCellDivCpuSingle::DSCalcCellDomain() {
	CellDomainMin = TUint3(0, 0, 0);
	CellDomainMax = TUint3(DomCells.x - 1, DomCells.y - 1, DomCells.z - 1);
	if (!BoundLimitOk) {
		BoundLimitOk = true; BoundLimitCellMin = CellDomainMin; BoundLimitCellMax = CellDomainMax;
...
void JCellDivCpuSingle::MergeMapCellBoundFluid(const tuint3& celbmin, const tuint3& celbmax, const tuint3& celfmin, const tuint3& celfmax, tuint3& celmin, tuint3& celmax)const {
	...
	celmax = TUint3(max(min(celbmin.x, celfmin.x), (celfmin.x >= scelldiv ? celfmin.x - scelldiv : 0))
	...
	celmin = TUint3(min(max(celbmax.x, celfmax.x), celfmax.x + scelldiv)
...
void JCellDivCpuSingle::DSPrepareNct() {
	//-Computes number of cells.
	Ncx = CellDomainMax.x - CellDomainMin.x + 1;
	Ncy = CellDomainMax.y - CellDomainMin.y + 1;
	Ncz = CellDomainMax.z - CellDomainMin.z + 1;
	...
	, unsigned* cellpart, unsigned* partsincell)const
{
	...
	for (unsigned p = 0; p < np; p++) {
		...
		if (codetype < CODE_TYPE_FLOATING) {//-Bound particles (except floating) | Particulas bound (excepto floating).
			box = (codeout < CODE_OUTIGNORE ?
				((cx < Ncx && cy < Ncy && cz < Ncz) ? cellsort : BoxBoundIgnore) :
				(codeout == CODE_OUTIGNORE ? BoxBoundOutIgnore : BoxBoundOut));
		}
		else {//-Fluid and floating particles | Particulas fluid y floating.
			box = (codeout <= CODE_OUTIGNORE ?
				(codeout < CODE_OUTIGNORE ? BoxFluid + cellsort : BoxFluidOutIgnore) :
				(codetype == CODE_TYPE_FLOATING ? BoxBoundOut : BoxFluidOut));
		}
		cellpart[p] = box;
		partsincell[box]++;
	}
...
/// Account for particles for cell (partsincell[]).
//==============================================================================
void JCellDivCpuSingle::DSPreSortFull(unsigned np, const unsigned* dcellc
	, unsigned* cellpart, unsigned* partsincell)const
{
	memset(partsincell, 0, sizeof(unsigned) * (Nctt - 1));
...
#ifdef OMP_USE
#pragma omp parallel for schedule (static) if(npint>OMP_LIMIT_COMPUTESTEPDEFSTRUC)
#endif
	for (int p = 0; p < npint; p++) {
		//-Computes cell according position.
		const unsigned rcell = dcellc[p];
		const unsigned cx = DCEL_Cellx(DomCellCode, rcell) - CellDomainMin.x;
		const unsigned cy = DCEL_Celly(DomCellCode, rcell) - CellDomainMin.y;
		...
		//-Assigns box.
		unsigned box = BoxFluid + cellsort;
	...
	, const typecode* codec, unsigned* cellpart, unsigned* partsincell)const
{
	...
	for (unsigned p = pini; p < pfin; p++) {
		...
		const typecode codeout = CODE_GetSpecialValue(rcode);
		//-Assigns box.
		const unsigned box = (codeout <= CODE_OUTIGNORE ?
			(codeout < CODE_OUTIGNORE ? cellsortfluid : BoxFluidOutIgnore) :
			(codetype == CODE_TYPE_FLOATING ? BoxBoundOut : BoxFluidOut));
		cellpart[p] = box;
		partsincell[box]++;
	}
...
void JCellDivCpuSingle::DSPreSort(const unsigned* dcellc) {

	DSPreSortFull(Nptot, dcellc, CellPart, PartsInCell);
	MakeSortFull(CellPart, BeginCell, PartsInCell, SortPart);
	SortArray(CellPart); //-Order values of CellPart[] | Ordena valores de CellPart[].
}
	...
	, const tdouble3* posc, JDsTimersCpu* timersc)
{
	...
	//printf("---> Nct:%u  BoxBoundOut:%u NpFinal=%u NpbFinal=%u\n", Nct, BoxBoundOut,NpFinal, NpbFinal);
...
//==============================================================================
/// Initial process of Divide: Calculate limits of domain and calculate new position
/// for each particle in deformable solid.
//==============================================================================
void JCellDivCpuSingle::DSDivide(unsigned np, const unsigned* dcellc, const typecode* codec, JDsTimersCpu* timersc)
{
	bool boundchanged = true;
	DivideFull = false;
	timersc->TmStart(TMC_NlLimits);

	//-Establish number of particles. | Establece numero de particulas.
	Npb1 = 0; Npf1 = np; Npb2 = 0; Npf2 = 0;
	Nptot = Npb1 + Npf1 + Npb2 + Npf2;
	NpbOut = NpfOut = NpbOutIgnore = NpfOutIgnore = 0;
	NpFinal = NpbFinal = 0;
	NpbIgnore = 0;
	DSCheckMemoryNp(Nptot);
	BoundLimitOk = BoundDivideOk = false;
	BoundLimitCellMin = BoundLimitCellMax = TUint3(0);
	BoundDivideCellMin = BoundDivideCellMax = TUint3(0);
