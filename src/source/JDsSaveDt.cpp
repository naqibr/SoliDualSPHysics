...
void JDsSaveDt::Reset(){
  ...
  LastDt3=ValueNull;                              //<vs_deformstruc>
  LastAceMax=LastViscDtMax=LastVelMax=ValueNull;
  LastDeformStrucDtMax=ValueNull;                   //<vs_deformstruc>
  CountAllDts=0;
}
...
void JDsSaveDt::SaveFileValues(){ 
  ...
  if(firstsv){
    ...
    scsv << "Dt3_mean [s];Dt3_min [s];Dt3_max [s]";                         //<vs_deformstruc>
    if(FullInfo){
      scsv << "AceMax_mean [m/s^2];AceMax_min [m/s^2];AceMax_max [m/s^2]";
      scsv << "ViscDtMax_mean;ViscDtMax_min;ViscDtMax_max";
      scsv << "DeformStrucDtMax_mean;DeformStrucDtMax_min;DeformStrucDtMax_max";  //<vs_deformstruc>
...
void JDsSaveDt::AddLastValues(){
  ...
  Dt1[Count]=LastDt1;
  Dt2[Count]=LastDt2;
  Dt3[Count]=LastDt3; //<vs_deformstruc>
  LastDtf=LastDt1=LastDt2=ValueNull;
  LastDt3=ValueNull;  //<vs_deformstruc>
  if(FullInfo){
    AceMax[Count]=LastAceMax;
    ViscDtMax[Count]=LastViscDtMax;
    DeformStrucDtMax[Count]=LastDeformStrucDtMax; //<vs_deformstruc>
...
/// Guarda info del dt inicado. Si coincide timestep lo sobre
//==============================================================================
void JDsSaveDt::AddValues(double timestep,double dtfinal,double dt1,double dt2,double dt3,double acemax,double viscdtmax,double deformstrucdtmax,double velmax){
  if(TimeStart<=timestep && timestep<=TimeFinish){
    ...
    AddValueData(timestep,dt3,LastDt3); //<vs_deformstruc>
    if(FullInfo){
      AddValueData(timestep,acemax,LastAceMax);
      AddValueData(timestep,viscdtmax,LastViscDtMax);
      AddValueData(timestep,deformstrucdtmax,LastDeformStrucDtMax); //<vs_deformstruc>
