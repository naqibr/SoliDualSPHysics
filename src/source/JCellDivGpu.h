...
protected:
	...
	unsigned* SortPart2;    ///<Copy of SortPart so can use in sort_by_key. //<vs_deformstruc>
	unsigned* SortIdx;      ///<Indices to particles which are sorted.      //<vs_deformstruc>
	...
	void DSCheckMemoryNp(unsigned npmin);
	void DSAllocMemoryNp(ullong np);
	void DSCheckMemoryNct(unsigned nctmin);
...
public:
	...
	void SortDataArrays(const unsigned* a1, const typecode* b1, const unsigned* c1, const float4* d1, unsigned* a2, typecode* b2, unsigned* c2, float4* d2);
	...
	void UpdateIndices(unsigned n, unsigned* idx); //<vs_deformstruc>
