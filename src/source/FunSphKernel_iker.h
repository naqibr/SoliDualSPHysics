...
//------------------------------------------------------------------------------
	...
	//============================================================================== 
	/// Returns kernel of TLSPH kernel according to input tker for variable kernelh of domains.
	//==============================================================================
	template<TpKernel tker, bool simulate2d> __device__ float GetTLSPHKernel_Wab(const float kernelh, const float rr2)
	{
		if (tker == KERNEL_Wendland) {
			float awen;
			if (simulate2d) awen = float(0.557 / (kernelh * kernelh));
			else awen = float(0.41778 / (kernelh * kernelh * kernelh));
			return(GetKernelWendland_Wab(rr2, kernelh, awen));
		}
		else if (tker == KERNEL_Cubic) {
			float a1, a2, a24;
			if (simulate2d) {
				a1 = float(10. / (PI * 7.));
				a2 = float(a1 / (kernelh * kernelh));
				a24 = float(0.25 * a2);
			}
			else {
				a1 = float(1. / PI);
				a2 = float(a1 / (kernelh * kernelh * kernelh));
				a24 = float(0.25 * a2);
			}
			const float rad = sqrt(rr2);
			const float qq = rad / kernelh;
			if (rad > kernelh) {
				const float wqq1 = 2.0f - qq;
				const float wqq2 = wqq1 * wqq1;
				return(a24 * (wqq2 * wqq1));
			}
			else {
				const float wqq2 = qq * qq;
				return(a2 * (1.0f + (0.75f * qq - 1.5f) * wqq2));
			}
		}
		else return(0);
	}
	...
	//============================================================================== 
	/// Returns fac of TLSPH kernel according to template for variable kernelh of domains.
	//==============================================================================
	template<TpKernel tker, bool simulate2d> __device__ float GetTLSPHKernel_Fac(const float kernelh, const float rr2)
	{
		if (tker == KERNEL_Wendland) {
			float bwen;
			if (simulate2d) bwen = float(-2.7852 / (kernelh * kernelh * kernelh));
			else bwen = float(-2.08891 / (kernelh * kernelh * kernelh * kernelh));
			return(GetKernelWendland_Fac(rr2, kernelh, bwen));
		}
		else if (tker == KERNEL_Cubic) {
			float c1, d1, c2, aa;
			if (simulate2d) {
				aa = float((10. / (PI * 7.)) / (kernelh * kernelh * kernelh));
				c1 = float(-3. * aa);
				d1 = float(9. * aa / 4.);
				c2 = float(-3. * aa / 4.);
			}
			else {
				aa = float((1. / PI) / (kernelh * kernelh * kernelh * kernelh));
				c1 = float(-3. * aa);
				d1 = float(9. * aa / 4.);
				c2 = float(-3. * aa / 4.);
			}
	...
	//============================================================================== 
	/// Returns wab and fac of TLSPH kernel according to template for variable kernelh of domains.
	//==============================================================================
	template<TpKernel tker, bool simulate2d> __device__ float GetTLSPHKernel_WabFac(const float kernelh, const float rr2, float& fac) {
		
		if (tker == KERNEL_Wendland) {
			float awen, bwen;
			if (simulate2d)
			{
				awen = float(0.557 / (kernelh * kernelh));
				bwen = float(-2.7852 / (kernelh * kernelh * kernelh));
			}
			else
			{
				awen = float(0.41778 / (kernelh * kernelh * kernelh));
				bwen = float(-2.08891 / (kernelh * kernelh * kernelh * kernelh));
			}
			const float rad = sqrt(rr2);
			const float qq = rad / kernelh;
			const float wqq1 = 1.f - 0.5f * qq;
			...
			float a1, a2, a24, aa, c1, d1, c2;
			if (simulate2d) {
				a1 = 10. / (PI * 7.);
				a2 = a1 / (kernelh * kernelh);
				a24 = float(0.25 * a2);
				aa = a1 / (kernelh * kernelh * kernelh);
				c1 = float(-3. * aa);
				d1 = float(9. * aa / 4.);
				c2 = float(-3. * aa / 4.);
			}
				...
				a1 = 1. / PI;
				a2 = a1 / (kernelh * kernelh * kernelh);
				a24 = float(0.25 * a2);
				aa = a1 / (kernelh * kernelh * kernelh * kernelh);
				c1 = float(-3. * aa);
				d1 = float(9. * aa / 4.);
				c2 = float(-3. * aa / 4.);
			}
