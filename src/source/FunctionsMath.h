...
//========================================================================================
	...
	static inline void SymmetrizeMatrix3x3(tmatrix3d& A)
	{
	...
	static inline tmatrix3d IdentMatrix3x3d()
	{
		tmatrix3d A;
		A.a11 = 1.0; A.a12 = 0.0; A.a13 = 0.0;
		A.a21 = 0.0; A.a22 = 1.0; A.a23 = 0.0;
		A.a31 = 0.0; A.a32 = 0.0; A.a33 = 1.0;
		return A;
	}

	static inline tmatrix3d ScaleMatrix3x3d(const tmatrix3d& A, const double s)
	{
		tmatrix3d B;
	...
	static inline tmatrix3d AddMatrix3x3d(const tmatrix3d& A, const tmatrix3d& B)
	{
		tmatrix3d C;
	...
	static inline double TraceMatrix3x3d(const tmatrix3d& A)
	{
		return A.a11 + A.a22 + A.a33;
	}

	static inline tmatrix3d DeviatoricMatrix3x3d(const tmatrix3d& A)
	{
		const double mean = TraceMatrix3x3d(A) / 3.0;
		tmatrix3d D = A;
		D.a11 -= mean;
		D.a22 -= mean;
		D.a33 -= mean;
	...
	static inline double SymmetricDoubleDotMatrix3x3d(const tmatrix3d& A, const tmatrix3d& B)
	{
		return
			A.a11 * B.a11 +
			A.a22 * B.a22 +
			A.a33 * B.a33 +
			2.0 * (A.a12 * B.a12 + A.a13 * B.a13 + A.a23 * B.a23);
	}

	static inline double MatrixInfNorm3x3d(const tmatrix3d& A)
	{
		const double r1 = std::fabs(A.a11) + std::fabs(A.a12) + std::fabs(A.a13);
		const double r2 = std::fabs(A.a21) + std::fabs(A.a22) + std::fabs(A.a23);
		const double r3 = std::fabs(A.a31) + std::fabs(A.a32) + std::fabs(A.a33);
		return std::fmax(r1, std::fmax(r2, r3));
	}

	static inline tmatrix3d ExpmMatrix3x3d(const tmatrix3d& A)
	{
		tmatrix3d B = A;
		int squarings = 0;

		double nrm = MatrixInfNorm3x3d(B);
		while (nrm > 0.5 && squarings < 12) {
			B = ScaleMatrix3x3d(B, 0.5);
			nrm *= 0.5;
			++squarings;
		}

		tmatrix3d E = IdentMatrix3x3d();
		tmatrix3d term = IdentMatrix3x3d();

		for (int k = 1; k <= 18; ++k) {
			term = fmath::MulMatrix3x3(term, B);
			term = ScaleMatrix3x3d(term, 1.0 / double(k));
			E = AddMatrix3x3d(E, term);
		}

		for (int i = 0; i < squarings; ++i) {
			E = fmath::MulMatrix3x3(E, E);
		}

		return E;
	}

	static inline void ComputeMandelDevAndEqStressd(
		const tmatrix3d& C,
		const double J,
		const tmatrix3d& Fp,
		const double mu,
		...
		double& sigma_eq)
	{
		tmatrix3d Fp_inv = fmath::InverseMatrix3x3(Fp);
		tmatrix3d Fp_invT = fmath::TrasMatrix3x3(Fp_inv);

		tmatrix3d tmpCe = fmath::MulMatrix3x3(C, Fp_inv);
		tmatrix3d Ce = fmath::MulMatrix3x3(Fp_invT, tmpCe);
		fmath::SymmetrizeMatrix3x3(Ce);

		const double Jp = std::fmax(fmath::Determinant3x3(Fp), 1e-12);
		const double Je = std::fmax(J / Jp, 1e-12);
		const double Je_pow = std::pow(Je, -2.0 / 3.0);

		tmatrix3d Ce_bar = ScaleMatrix3x3d(Ce, Je_pow);
		tmatrix3d devCe_bar = DeviatoricMatrix3x3d(Ce_bar);

		M_dev = ScaleMatrix3x3d(devCe_bar, mu);
		fmath::SymmetrizeMatrix3x3(M_dev);

		const double M2 = SymmetricDoubleDotMatrix3x3d(M_dev, M_dev);
		sigma_eq = std::sqrt(std::fmax(1.5 * M2, 0.0));
	}

	static inline double J2ResidualAtDgammad(
		const double dgamma,
		const tmatrix3d& Nflow_trial,
		const tmatrix3d& Fp_n,
		const tmatrix3d& C,
		const double J,
		const double mu,
		...
		tmatrix3d* Fp_out)
	{
		tmatrix3d A = ExpmMatrix3x3d(ScaleMatrix3x3d(Nflow_trial, dgamma));
		tmatrix3d Fp = fmath::MulMatrix3x3(A, Fp_n);

		double detFp = std::fmax(fmath::Determinant3x3(Fp), 1e-12);
		const double siso = std::pow(detFp, -1.0 / 3.0);
		Fp = ScaleMatrix3x3d(Fp, siso);

		if (!std::isfinite(detFp) || detFp <= 0.0) {
		...
		tmatrix3d M_dev;
		double sigma_eq = 0.0;
		ComputeMandelDevAndEqStressd(C, J, Fp, mu, M_dev, sigma_eq);

		if (Fp_out) *Fp_out = Fp;

		const double alpha = alpha_n + dgamma;
		const double sigma_y = std::fmax(sigma_y0 + H * alpha, 0.0);
		return sigma_eq - sigma_y;
	}

	static inline double SolveDgammaIllinoisd(
		const tmatrix3d& Nflow_trial,
		const tmatrix3d& Fp_n,
		const tmatrix3d& C,
		const double J,
		...
		const double H,
		const double alpha_n,
		const double sigma_eq_trial,
		const double sigma_y_trial,
		tmatrix3d* Fp_sol)
	{
		double xL = 0.0;
		double fL = sigma_eq_trial - sigma_y_trial;

		double xR = (sigma_eq_trial - sigma_y_trial) / std::fmax(3.0 * mu + H, 1e-12);
		xR = std::fmax(xR, 1e-8);

		tmatrix3d FpR;
		double fR = J2ResidualAtDgammad(
			xR, Nflow_trial, Fp_n, C, J, mu, sigma_y0, H, alpha_n, &FpR);

		const double ftolR = 1e-5 * std::fmax(sigma_y0 + H * (alpha_n + xR), 1.0);
		if (std::fabs(fR) < ftolR) {
			if (Fp_sol) *Fp_sol = FpR;
			return xR;
		}

		for (int k = 0; k < 20 && fR > 0.0; ++k) {
			xR *= 2.0;
			fR = J2ResidualAtDgammad(
				xR, Nflow_trial, Fp_n, C, J, mu, sigma_y0, H, alpha_n, &FpR);

			const double ftol_expand = 1e-5 * std::fmax(sigma_y0 + H * (alpha_n + xR), 1.0);
			if (std::fabs(fR) < ftol_expand) {
				if (Fp_sol) *Fp_sol = FpR;
				return xR;
			}
		}
		...
		for (int it = 0; it < 20; ++it) {
			const double denom = (fR - fL);
			double x = (std::fabs(denom) > 1e-20)
				? (xR - fR * (xR - xL) / denom)
				: (0.5 * (xL + xR));

			if (!(x > xL && x < xR) || !std::isfinite(x)) {
				x = 0.5 * (xL + xR);
			}

			const double f = J2ResidualAtDgammad(
				x, Nflow_trial, Fp_n, C, J, mu, sigma_y0, H, alpha_n, &FpX);

			const double xtol = 1e-7 * (1.0 + xR);
			const double ftol = 1e-5 * std::fmax(sigma_y0 + H * (alpha_n + x), 1.0);

			if (std::fabs(f) < ftol || std::fabs(xR - xL) < xtol) {
