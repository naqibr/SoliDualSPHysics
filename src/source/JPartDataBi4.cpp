...
void JPartDataBi4::AddPartData(unsigned npok, const unsigned* idp, const ullong* idpd
	...
	, const float* pressp, bool externalpointer)
{
	...
	if(rhop) Part->CreateArray("Rhop", JBinaryDataDef::DatFloat, npok, rhop, externalpointer);
	if (pressp) Part->CreateArray("Press", JBinaryDataDef::DatFloat, npok, pressp, externalpointer);
}
