...
static void WriteEnergiesCsv(const std::string& file, bool csvComma, double timestep,
    ...
    if (table.header.empty()) {
        ...
        for (size_t i = 0; i < bodies.size(); ++i) {
            ...
            } else if(bodies[i].constitutive == CONSTITMODEL_J2) {
                table.header.push_back(prefix + "_PLASTIC_WORK");
			}
            table.fracture[i] = bodies[i].fracture;
			//table.plastic[i] = (bodies[i].constitutive == CONSTITMODEL_J2);
        }
    }
    ...
    for (size_t i = 0; i < bodies.size(); ++i) {
        ...
        if (bodies[i].constitutive == CONSTITMODEL_J2) row.push_back(fun::PrintStr("%.15g", energies[i].z));
    }
...
#pragma omp parallel for if(dfnp > 5000)
            ...
            if ((fieldMask & FieldEnergy) && !strainEnergy.empty()) {
                ...
                if (!fractureOrPlasticEnergy.empty()) {
                    if (body.fracture) {
                        ...
                        arraysdef.AddArray("Plastic_Work", dfnp, fractureOrPlasticEnergy.data());
                    }
                }
            }
        ...
        if (!energyOut.empty()) WriteEnergiesCsv(energyFile, csvComma, timestep, bodies, energyOut);
